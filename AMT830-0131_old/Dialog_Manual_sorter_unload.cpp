// Dialog_Manual_sorter_unload.cpp : implementation file
//
...
CDialog_Manual_sorter_unload::CDialog_Manual_sorter_unload(CWnd* pParent /*=NULL*/)
	: CDialog(CDialog_Manual_sorter_unload::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDialog_Manual_sorter_unload)
...
void CDialog_Manual_sorter_unload::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDialog_Manual_sorter_unload)
	...
	DDX_Control(pDX, IDC_CMB_TRAY_POS2, m_cmb_tray_pos2);
	...
	DDX_Control(pDX, IDC_BTN_GO_TRAY_POS2, m_btn_go_tray_pos2);
	...
	DDX_Control(pDX, IDC_BTN_BUFFER_SORTER_SOL4, m_btn_buffer_sorter_sol4);
	DDX_Control(pDX, IDC_BTN_BUFFER_SORTER_SOL3, m_btn_buffer_sorter_sol3);
	DDX_Control(pDX, IDC_BTN_BUFFER_SORTER_SOL2, m_btn_buffer_sorter_sol2);
	DDX_Control(pDX, IDC_BTN_BUFFER_SORTER_SOL1, m_btn_buffer_sorter_sol1);
	...
	DDX_Control(pDX, IDC_GROUP_RBT_MOVE2, m_group_picker_rbt_move2);
	DDX_Control(pDX, IDC_GROUP_PICKER, m_group_picker);
	DDX_Control(pDX, IDC_GROUP_SORTER_RBT_MOVE, m_group_sorter_rbt_move);
	DDX_Control(pDX, IDC_CMB_SORTER_POS, m_cmb_sorter_pos);
	DDX_Control(pDX, IDC_CMB_UNLOADER_POS, m_cmb_unloader_pos);
	DDX_Control(pDX, IDC_BTN_GO_SORTER_POS, m_btn_go_sorter_pos);
	DDX_Control(pDX, IDC_MSG_PICKER_SOL2, m_msg_picker_sol2);
	DDX_Control(pDX, IDC_MSG_FINGER_SOL2, m_msg_finger_sol2);
	DDX_Control(pDX, IDC_BTN_GO_STEP_RBT_MOVE2, m_btn_go_step_rbt_move2);
	DDX_Control(pDX, IDC_BTN_GO_UNLOADER_POS, m_btn_go_unloader_pos);
	DDX_Control(pDX, IDC_GROUP_CYLINDER2, m_group_cylinder2);
	DDX_Control(pDX, IDC_BTN_CYLINDER_SOL2, m_btn_cylinder_sol2);
	DDX_Control(pDX, IDC_MSG_CYLINDER_SOL2, m_msg_cylinder_sol2);
...
BEGIN_MESSAGE_MAP(CDialog_Manual_sorter_unload, CDialog)
	//{{AFX_MSG_MAP(CDialog_Manual_sorter_unload)
	...
	ON_BN_CLICKED(IDC_BTN_GO_STEP_RBT_MOVE2, OnBtnGoStepRbtMove2)
	...
	ON_BN_CLICKED(IDC_BTN_GO_SORTER_POS, OnBtnGoSorterPos)
	ON_BN_CLICKED(IDC_BTN_GO_BUFFER_POS, OnBtnGoBufferPos)
	ON_BN_CLICKED(IDC_BTN_BUFFER_SORTER_SOL1, OnBtnBufferSorterSol1)
	ON_BN_CLICKED(IDC_BTN_BUFFER_SORTER_SOL2, OnBtnBufferSorterSol2)
	ON_BN_CLICKED(IDC_BTN_BUFFER_SORTER_SOL3, OnBtnBufferSorterSol3)
	ON_BN_CLICKED(IDC_BTN_BUFFER_SORTER_SOL4, OnBtnBufferSorterSol4)
	ON_BN_CLICKED(IDC_BTN_PICKER_SOL_ALL, OnBtnPickerSolAll)
	ON_BN_CLICKED(IDC_BTN_FINGER_SOL_ALL, OnBtnFingerSolAll)
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_BTN_GO_UNLOADER_POS, OnBtnGoUnloaderPos)
	ON_BN_CLICKED(IDC_BTN_GO_TRAY_POS2, OnBtnGoTrayPos2)
	ON_BN_CLICKED(IDC_BTN_CYLINDER_SOL2, OnBtnCylinderSol2)
	//}}AFX_MSG_MAP
/////////////////////////////20121006
...
/////////////////////////////////////////////////////////////////////////////
// CDialog_Manual_sorter_unload message handlers

BOOL CDialog_Manual_sorter_unload::OnInitDialog() 
{
...
////////////////////////////////////////////////////
	...
	if(nMotorPos == 0)
	{
		m_n_axis = M_RBT_UNLOAD_X;
	}
	else if(nMotorPos == 1)
	{
		m_n_axis = M_RBT_UNLOAD_Y;
	}
	else if(nMotorPos == 2)
	{
		m_n_axis = M_RBT_UNLOAD_Z;
	}
	else if(nMotorPos == 3)
	{
		m_n_axis = M_RBT_SORTER_Z;
	}
	else if(nMotorPos == 4)
	{
		m_n_axis = M_RBT_SORTER_X;
	}
	else if(nMotorPos == 5)
	{
		m_n_axis = M_RBT_SORTER_Y;
	}
	else if(nMotorPos == 6)
	{
		m_n_axis = M_ULDM_STACKER_1;
	}
	else if(nMotorPos == 7)
	{
		m_n_axis = M_ULDM_STACKER_2;
	}
	else if(nMotorPos == 8)
	{
		m_n_axis = M_ULDM_STACKER_MOVE;
	}
	else
	{
		m_n_axis = M_WORK_BUFFER_1 + nMotorPos - 9;
	}
	...
	SetTimer(TMR_SORTERUNLOAD_IO_MONITOR_CMD, 50, NULL);		// IO 정보 읽기 타이머 생성
	return TRUE;  // return TRUE unless you set the focus to a control
	              // EXCEPTION: OCX Property Pages should return FALSE
}

	//////////////////////////////////////////////////// 20121006
void CDialog_Manual_sorter_unload::Init_Grid_Info()
{
	int   i, j;
	CString str_tmp;
...
void CDialog_Manual_sorter_unload::Init_GroupBox_Set()
{
	...
	m_group_picker_rbt_move2.SetFont(ls_Logo);
	m_group_picker_rbt_move2.SetCatptionTextColor(RGB(145,25,0));
	m_group_picker_rbt_move2.SetFontBold(TRUE);
	...
	m_group_cylinder2.SetFont(ls_Logo);
	m_group_cylinder2.SetCatptionTextColor(RGB(145,25,0));
	m_group_cylinder2.SetFontBold(TRUE);

}

void CDialog_Manual_sorter_unload::Init_Label()
{
	int i;
	CString str_line;
	...
	m_cmb_tray_pos2.SetFont(m_load_combo_font);
	m_cmb_tray_pos2.PostMessage(CB_SETITEMHEIGHT, (WPARAM)-1, (LPARAM) 60);
	
	for (i = 0; i < st_traybuffer[UNLOADER_SITE].i_loader_row; i++)
	{
		str_line.Format("%02d", i + 1);		
		m_cmb_tray_pos2.AddString(str_line);
	}

	m_cmb_tray_pos2.SetCurSel(0);
	...
	m_cmb_sorter_pos.SetFont(m_p_font);
	m_cmb_sorter_pos.PostMessage(CB_SETITEMHEIGHT, (WPARAM)-1, (LPARAM) 20);
	...
	for (i = 0; i < 21; i++)
	{
		...
		{
			str_line.Format("Z_WB%01d", i);				
		}
		else if(i == 5)
		{
			str_line.Format("Z_Vision1");				
		}
		else if(i == 6)
		{
			str_line.Format("Z_Vision2");				
		}
		else if(i == 7)
		{
		...
		{
			str_line.Format("Y_WB%01d", i - 7);				
		}
		else if(i == 12)
		{
			str_line.Format("Y_Vision1");				
		}
		else if(i == 13)
		{
			str_line.Format("Y_Vision2");				
		}
		else if(i == 14)
		{
			str_line.Format("X_Safety");				
		}
		else if(i == 15 || i == 16 || i == 17 || i == 18)
		{
			str_line.Format("X_WB%01d", i - 14);				
		}
		else if(i == 19)
		{
			str_line.Format("X_Vision1");				
		}
		else if(i == 20)
		{
			str_line.Format("X_Vision2");				
		}

		m_cmb_sorter_pos.AddString(str_line);
	}
	m_cmb_sorter_pos.SetCurSel(0);



	m_cmb_unloader_pos.SetFont(m_p_font);
	m_cmb_unloader_pos.PostMessage(CB_SETITEMHEIGHT, (WPARAM)-1, (LPARAM) 20);
	...
	for (i = 0; i < 36; i++)
	{
		...
		{
			str_line.Format("X_Uld_Tray_Start");				
		}
		else if(i == 2)
		{
			str_line.Format("X_Uld_Tray_End");				
		}
		else if(i == 3)
		{
			str_line.Format("X_Vision1");				
		...
		{
			str_line.Format("X_Reject_Tray_Start");				
		}
		else if(i == 5)
		{
			str_line.Format("X_Reject_Tray_End");				
		}
		else if(i == 6)
		{
			str_line.Format("X_Vision2");				
		...
		{
			str_line.Format("Y_Uld_Tray_Start");				
		}
		else if(i == 9)
		{
			str_line.Format("Y_Uld_Tray_End");				
		}
		else if(i == 10)
		{
			str_line.Format("Y_Vision1");				
		...
		{
			str_line.Format("Y_Reject_Tray_Start");				
		}
		else if(i == 12)
		{
			str_line.Format("Y_Reject_Tray_End");				
		}
		else if(i == 13)
		{
			str_line.Format("Y_Vision2");				
		...
		{
			str_line.Format("Z_Uld_Tray_Start");				
		}
		else if(i == 16)
		{
			str_line.Format("Z_Uld_Tray_End");				
		}
		else if(i == 17)
		{
			str_line.Format("Z_Vision1");				
		...
		{
			str_line.Format("Z_Reject_Tray_Start");				
		}
		else if(i == 19)
		{
			str_line.Format("Z_Reject_Tray_End");				
		}
		else if(i == 20)
		{
			str_line.Format("Z_Vision2");				
		...
		m_cmb_unloader_pos.AddString(str_line);
	}
	m_cmb_unloader_pos.SetCurSel(0);
	...
	for (i = 0; i < 13; i++)
	{
		if(i == 0)
		{
			str_line.Format("%02d_UNLOAD_RBT_X", i + 1);
		}
		else if(i == 1)
		{
			str_line.Format("%02d_UNLOAD_RBT_Y", i + 1);
		}
		else if(i == 2)
		{
			str_line.Format("%02d_UNLOAD_RBT_Z", i + 1);
		}
		else if(i == 3)
		{
			str_line.Format("%02d_SORTER_RBT_Z", i + 1);
		}
		else if(i == 4)
		{
			str_line.Format("%02d_SORTER_RBT_X", i + 1);
		}
		else if(i == 5)
		{
			str_line.Format("%02d_SORTER_RBT_Y", i + 1);
		}
		else if(i == 6)
		{
		...
		{
			str_line.Format("BUFFER_%02d", i - 8);
		}
	...
	m_msg_picker_sol2.SetFont(m_p_font);
	m_msg_picker_sol2.SetWindowText(_T("Picker sol"));
	m_msg_picker_sol2.SetCenterText();
	m_msg_picker_sol2.SetColor(RGB(0,0,255));
	m_msg_picker_sol2.SetGradientColor(RGB(0,0,0));
	m_msg_picker_sol2.SetTextColor(RGB(255,255,255));

	m_msg_finger_sol2.SetFont(m_p_font);
	m_msg_finger_sol2.SetWindowText(_T("Finger sol"));
	m_msg_finger_sol2.SetCenterText();
	m_msg_finger_sol2.SetColor(RGB(0,0,255));
	m_msg_finger_sol2.SetGradientColor(RGB(0,0,0));
	m_msg_finger_sol2.SetTextColor(RGB(255,255,255));
	...
	m_msg_picker2.SetWindowText(_T("#1"));
	...
	m_msg_cylinder_sol2.SetFont(m_p_font);
	m_msg_cylinder_sol2.SetWindowText(_T("cylinder Up/Down"));
	m_msg_cylinder_sol2.SetCenterText();
	m_msg_cylinder_sol2.SetColor(RGB(0,0,255));
	m_msg_cylinder_sol2.SetGradientColor(RGB(0,0,0));
	m_msg_cylinder_sol2.SetTextColor(RGB(255,255,255));

}

void CDialog_Manual_sorter_unload::Init_Data()
{
 	int i;
	...
	for (i = 0; i < 2; i++)
	{
		...
		mn_finger[i] = 0;
		mn_cylinder2[i] = 0;
		mn_sorter_buffer[i] = 0;
		mn_sorter_buffer[i + 2] = 0;
	}

//////////////////////////////////////////////////// 20121006
	m_n_axis			= st_motor_info.n_part_axis[5][AXIS_NUM];
		
	m_str_axis_name = st_motor_info.str_part_axis_name[5][AXIS_NUM];
	m_n_minus_el	= st_motor_info.n_axis_minus_el[5][AXIS_NUM];
	m_n_plus_el		= st_motor_info.n_axis_plus_el[5][AXIS_NUM];
	m_n_home		= st_motor_info.n_axis_home[5][AXIS_NUM];
	m_n_sd			= st_motor_info.n_axis_sd[5][AXIS_NUM];
	m_n_direction	= st_motor_info.n_axis_direction[5][AXIS_NUM];
	m_n_account		= st_motor_info.n_axis_account[5][AXIS_NUM];
////////////////////////////////////////////////////	
}

void CDialog_Manual_sorter_unload::Init_Button()
{
	short	shBtnColor = 30;
	CString strTmp;
	int nMotorPos = m_cmb_motor_pos.GetCurSel();

	if(nMotorPos == 1 || nMotorPos == 5 || nMotorPos == 6 || nMotorPos == 7 || nMotorPos == 8)
	{
		m_btn_left.SetIcon(IDI_DN);
	}
	else if(nMotorPos == 2 || nMotorPos == 3)
	{
...
// 	m_btn_right.SetIcon(IDI_UP);
	...
	m_btn_buffer_sorter_sol1.SetFont(m_p_font);
	m_btn_buffer_sorter_sol1.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
	m_btn_buffer_sorter_sol1.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
	m_btn_buffer_sorter_sol1.SetButtonColor(1);
	
	m_btn_buffer_sorter_sol2.SetFont(m_p_font);
	m_btn_buffer_sorter_sol2.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
	m_btn_buffer_sorter_sol2.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
	m_btn_buffer_sorter_sol2.SetButtonColor(1);
	
	m_btn_buffer_sorter_sol3.SetFont(m_p_font);
	m_btn_buffer_sorter_sol3.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
	m_btn_buffer_sorter_sol3.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
	m_btn_buffer_sorter_sol3.SetButtonColor(1);
	
	m_btn_buffer_sorter_sol4.SetFont(m_p_font);
	m_btn_buffer_sorter_sol4.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
	m_btn_buffer_sorter_sol4.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
	m_btn_buffer_sorter_sol4.SetButtonColor(1);
	
	m_btn_cylinder_sol2.SetFont(m_p_font);
	m_btn_cylinder_sol2.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
	m_btn_cylinder_sol2.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
	m_btn_cylinder_sol2.SetButtonColor(1);
	
}

void CDialog_Manual_sorter_unload::OnBtnPickerSol1() 
{
	// TODO: Add your control notification handler code here
	if (m_btn_picker_sol1.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_sorter_picker_rbt_updn_onoff, IO_ON);
		mn_picker[0] = TRUE;
	}
	else
	{
		FAS_IO.set_out_bit(st_io.o_sorter_picker_rbt_updn_onoff, IO_OFF);
		mn_picker[0] = FALSE;
	}	
	OnSiteRbt_Button_Status();	
}

void CDialog_Manual_sorter_unload::OnBtnPickerSol2() 
{
	// TODO: Add your control notification handler code here
	if (m_btn_picker_sol2.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_uld_picker_rbt_updn_onoff, IO_ON);
		mn_picker[1] = TRUE;
	}
	else
	{
		FAS_IO.set_out_bit(st_io.o_uld_picker_rbt_updn_onoff, IO_OFF);
		mn_picker[1] = FALSE;
	}	
	OnSiteRbt_Button_Status();		
...
void CDialog_Manual_sorter_unload::OnBtnPickerSolAll() 
{
	// TODO: Add your control notification handler code here
	
}

void CDialog_Manual_sorter_unload::OnBtnFingerSol1() 
{
	// TODO: Add your control notification handler code here
	if (m_btn_finger_sol1.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_sorter_picker_rbt_gripper_onoff, IO_OFF);
		mn_finger[0] = TRUE;
	}
	else
	{
...
void CDialog_Manual_sorter_unload::OnBtnFingerSol2() 
{
	// TODO: Add your control notification handler code here
	if (m_btn_finger_sol2.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_uld_picker_rbt_gripper_onoff, IO_OFF);
		mn_finger[1] = TRUE;
	}
	else
	{
...
void CDialog_Manual_sorter_unload::OnBtnFingerSolAll() 
{
	// TODO: Add your control notification handler code here
	
}

void CDialog_Manual_sorter_unload::OnBtnBufferSorterSol1() 
{
	// TODO: Add your control notification handler code here
	if (m_btn_buffer_sorter_sol1.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_buf1_m_clamp_onoff, IO_ON);
		mn_sorter_buffer[0] = TRUE;
	}
	else
	{
		FAS_IO.set_out_bit(st_io.o_buf1_m_clamp_onoff, IO_OFF);
		mn_sorter_buffer[0] = FALSE;
	}	
	OnSiteRbt_Button_Status();
	
}

void CDialog_Manual_sorter_unload::OnBtnBufferSorterSol2() 
{
	// TODO: Add your control notification handler code here
	if (m_btn_buffer_sorter_sol2.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_buf2_m_clamp_onoff, IO_ON);
		mn_sorter_buffer[1] = TRUE;
	}
	else
	{
		FAS_IO.set_out_bit(st_io.o_buf2_m_clamp_onoff, IO_OFF);
		mn_sorter_buffer[1] = FALSE;
	}	
	OnSiteRbt_Button_Status();		
}

void CDialog_Manual_sorter_unload::OnBtnBufferSorterSol3() 
{
	// TODO: Add your control notification handler code here
	if (m_btn_buffer_sorter_sol3.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_buf3_m_clamp_onoff, IO_ON);
		mn_sorter_buffer[2] = TRUE;
	}
	else
	{
		FAS_IO.set_out_bit(st_io.o_buf3_m_clamp_onoff, IO_OFF);
		mn_sorter_buffer[2] = FALSE;
	}	
	OnSiteRbt_Button_Status();	
	
}

void CDialog_Manual_sorter_unload::OnBtnBufferSorterSol4() 
{
	// TODO: Add your control notification handler code here
	if (m_btn_buffer_sorter_sol4.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_buf4_m_clamp_onoff, IO_ON);
		mn_sorter_buffer[3] = TRUE;
	}
	else
	{
		FAS_IO.set_out_bit(st_io.o_buf4_m_clamp_onoff, IO_OFF);
		mn_sorter_buffer[3] = FALSE;
	}	
	OnSiteRbt_Button_Status();	
	
}

void CDialog_Manual_sorter_unload::OnRead_IO_Check()
{
	int i;

	if(FAS_IO.get_in_bit(st_io.o_uld_stacker1_up_onoff, IO_ON) == IO_ON && 
		FAS_IO.get_in_bit(st_io.o_uld_stacker1_dn_onoff, IO_OFF) == IO_OFF)
	{
		mn_cylinder2[0] = TRUE;
	}
	else
	{
		mn_cylinder2[0] = FALSE;		
	}

	if(FAS_IO.get_in_bit(st_io.o_sorter_picker_rbt_updn_onoff, IO_ON) == IO_ON)
	{
		mn_picker[0] = TRUE;
	}
	else
	...
	if(FAS_IO.get_in_bit(st_io.o_uld_picker_rbt_updn_onoff, IO_ON) == IO_ON)
	{
		mn_picker[1] = TRUE;
	}
	else
	...
	if(FAS_IO.get_in_bit(st_io.o_sorter_picker_rbt_gripper_onoff, IO_ON) == IO_ON)
	{
		mn_finger[0] = FALSE;
	}
	else
	...
	if(FAS_IO.get_in_bit(st_io.o_uld_picker_rbt_gripper_onoff, IO_ON) == IO_ON)
	{
		mn_finger[1] = FALSE;
	}
	else
	...
	for(i=0; i<4; i++)
	{
		...
		{
			mn_sorter_buffer[i] = TRUE;
		}
		else
		{
			mn_sorter_buffer[i] = FALSE;		
		}	
	}
...
void CDialog_Manual_sorter_unload::OnSiteRbt_Button_Status()//20121006
{
	short	shBtnColor = 30;
	
	if (mn_cylinder2[0] < 0 || mn_cylinder2[0] > 1) mn_cylinder2[0] = TRUE;
	m_btn_cylinder_sol2.SetCheck(mn_cylinder2[0]);
	if (mn_cylinder2[0] == TRUE)
	{
		m_btn_cylinder_sol2.SetCheck(TRUE);
		SetDlgItemText(IDC_BTN_CYLINDER_SOL2, "Down");
		m_btn_cylinder_sol2.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_cylinder_sol2.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_cylinder_sol2.SetButtonColor(1);
	}
	...
	{
		m_btn_cylinder_sol2.SetCheck(FALSE);
		SetDlgItemText(IDC_BTN_CYLINDER_SOL2, "Up");
		m_btn_cylinder_sol2.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_cylinder_sol2.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_cylinder_sol2.SetButtonColor(1);
	...
	if (mn_sorter_buffer[0] < 0 || mn_sorter_buffer[0] > 1) mn_sorter_buffer[0] = TRUE;
	m_btn_buffer_sorter_sol1.SetCheck(mn_sorter_buffer[0]);
	if (mn_sorter_buffer[0] == TRUE)
	{
		m_btn_buffer_sorter_sol1.SetCheck(TRUE);
		SetDlgItemText(IDC_BTN_BUFFER_SORTER_SOL1, "On");
		m_btn_buffer_sorter_sol1.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_buffer_sorter_sol1.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_buffer_sorter_sol1.SetButtonColor(1);
	...
	{
		m_btn_buffer_sorter_sol1.SetCheck(FALSE);
		SetDlgItemText(IDC_BTN_BUFFER_SORTER_SOL1, "Off");
		m_btn_buffer_sorter_sol1.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_buffer_sorter_sol1.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_buffer_sorter_sol1.SetButtonColor(1);
	...
	if (mn_sorter_buffer[1] < 0 || mn_sorter_buffer[1] > 1) mn_sorter_buffer[1] = TRUE;
	m_btn_buffer_sorter_sol2.SetCheck(mn_sorter_buffer[1]);
	if (mn_sorter_buffer[1] == TRUE)
	{
		m_btn_buffer_sorter_sol2.SetCheck(TRUE);
		SetDlgItemText(IDC_BTN_BUFFER_SORTER_SOL2, "On");
		m_btn_buffer_sorter_sol2.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_buffer_sorter_sol2.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_buffer_sorter_sol2.SetButtonColor(1);
	...
	{
		m_btn_buffer_sorter_sol2.SetCheck(FALSE);
		SetDlgItemText(IDC_BTN_BUFFER_SORTER_SOL2, "Off");
		m_btn_buffer_sorter_sol2.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_buffer_sorter_sol2.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_buffer_sorter_sol2.SetButtonColor(1);
	...
	if (mn_sorter_buffer[2] < 0 || mn_sorter_buffer[2] > 1) mn_sorter_buffer[2] = TRUE;
	m_btn_buffer_sorter_sol3.SetCheck(mn_sorter_buffer[2]);
	if (mn_sorter_buffer[2] == TRUE)
	{
		m_btn_buffer_sorter_sol3.SetCheck(TRUE);
		SetDlgItemText(IDC_BTN_BUFFER_SORTER_SOL3, "On");
		m_btn_buffer_sorter_sol3.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_buffer_sorter_sol3.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_buffer_sorter_sol3.SetButtonColor(1);
	...
	{
		m_btn_buffer_sorter_sol3.SetCheck(FALSE);
		SetDlgItemText(IDC_BTN_BUFFER_SORTER_SOL3, "Off");
		m_btn_buffer_sorter_sol3.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_buffer_sorter_sol3.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_buffer_sorter_sol3.SetButtonColor(1);
	...
	if (mn_sorter_buffer[3] < 0 || mn_sorter_buffer[3] > 1) mn_sorter_buffer[3] = TRUE;
	m_btn_buffer_sorter_sol4.SetCheck(mn_sorter_buffer[3]);
	if (mn_sorter_buffer[3] == TRUE)
	{
		m_btn_buffer_sorter_sol4.SetCheck(TRUE);
		SetDlgItemText(IDC_BTN_BUFFER_SORTER_SOL4, "On");
		m_btn_buffer_sorter_sol4.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_buffer_sorter_sol4.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_buffer_sorter_sol4.SetButtonColor(1);
	...
	{
		m_btn_buffer_sorter_sol4.SetCheck(FALSE);
		SetDlgItemText(IDC_BTN_BUFFER_SORTER_SOL4, "Off");
		m_btn_buffer_sorter_sol4.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_buffer_sorter_sol4.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_buffer_sorter_sol4.SetButtonColor(1);
...
void CDialog_Manual_sorter_unload::OnSite_Controls_Enable(BOOL iFlag)
{
	m_btn_cylinder_sol2.EnableWindow(iFlag);
	...
	m_btn_buffer_sorter_sol1.EnableWindow(iFlag);
	m_btn_buffer_sorter_sol2.EnableWindow(iFlag);
	m_btn_buffer_sorter_sol3.EnableWindow(iFlag);
	m_btn_buffer_sorter_sol4.EnableWindow(iFlag);
	
	m_btn_go_sorter_pos.EnableWindow(iFlag);
	m_btn_go_tray_pos.EnableWindow(iFlag);
	m_btn_go_tray_pos2.EnableWindow(iFlag);
	m_cmb_buffer_pos.EnableWindow(iFlag);
	m_cmb_sorter_pos.EnableWindow(iFlag);
	m_cmb_tray_pos.EnableWindow(iFlag);	
	m_cmb_tray_pos2.EnableWindow(iFlag);	
	m_btn_go_unloader_pos.EnableWindow(iFlag);

}

void CDialog_Manual_sorter_unload::OnSite_Step_Clear()
{
...
void CDialog_Manual_sorter_unload::OnBtnGoTrayPos() 
{
	int n_tray_y;
	...
	if (n_tray_y < 0)	
	{
		...
		{
			sprintf(st_msg.c_abnormal_msg,"[Unloade RBT] can not move.");
	...
	////////////////////// 20121107
	nRet = COMI.Check_SingleMove(M_RBT_UNLOAD_Z, Z_LD_SAFETY_UP);
	...
	md_UldRobot_Y_Pos = Func.XYRobot_MovePosValCheck(UNLOADER_SITE, 0, n_tray_y);
	dUldAbsPos[0] = md_UldRobot_X_Pos;
	dUldAbsPos[1] = md_UldRobot_Y_Pos;
	
	SetTimer(TMR_UNLOAD_TRAY_MOT_CMD, 10, NULL);		// 모터 동작 제어 타이머
	// **************************************************************************	
}

void CDialog_Manual_sorter_unload::OnBtnGoTrayPos2() 
{
	int n_tray_y;
	int mn_FirstPicker_Y_Pos = 0;
	double dSpeedRatio[3] = {st_basic.nRunSpeed,st_basic.nRunSpeed,st_basic.nRunSpeed};
	...
	n_tray_y = m_cmb_tray_pos2.GetCurSel();
	
	if (n_tray_y < 0)	
	{
		if (st_handler.cwnd_list != NULL)  // 리스트 바 화면 존재
		{
			sprintf(st_msg.c_abnormal_msg,"[Reject RBT] can not move.");
			st_handler.cwnd_list->PostMessage(WM_LIST_DATA, 0, ABNORMAL_MSG);  // 동작 완료 출력 요청
		}
		return;
	}
	...
	md_UldRobot_Y_Pos = Func.XYRobot_MovePosValCheck(REJECT_SITE, 0, n_tray_y);
	dUldAbsPos[0] = md_UldRobot_X_Pos;
	dUldAbsPos[1] = md_UldRobot_Y_Pos;
...
void CDialog_Manual_sorter_unload::OnBtnGoSorterPos() 
{
	// TODO: Add your control notification handler code here
	int n_tray_y;
	int mn_FirstPicker_Y_Pos = 0;
	...
	n_tray_y = m_cmb_sorter_pos.GetCurSel();
	
	if (n_tray_y < 0)	
	{
		if (st_handler.cwnd_list != NULL)  // 리스트 바 화면 존재
		{
			sprintf(st_msg.c_abnormal_msg,"[Sorter RBT] can not move.");
			st_handler.cwnd_list->PostMessage(WM_LIST_DATA, 0, ABNORMAL_MSG);  // 동작 완료 출력 요청
		}
		return;
	}
	...
	////////////////////// 20121107
	if(n_tray_y == 0 || n_tray_y == 1 || n_tray_y == 2 ||
	   n_tray_y == 3 || n_tray_y == 4 || n_tray_y == 5 || n_tray_y == 6)
	{
	}
	else
	{
		nRet = COMI.Check_SingleMove(M_RBT_SORTER_Z, Z_SORT_SAFETY_UP);
		if (nRet != CTLBD_RET_GOOD)
		{
			st_msg.str_fallacy_msg = _T("Z축이 Safety 위치가 아닙니다.");	
			if(st_handler.mn_language == LANGUAGE_ENGLISH)
...
//	mn_motor_no = M_RBT_SORTER_Z;
	if(n_tray_y == 0 || n_tray_y == 1 || n_tray_y == 2 ||
	   n_tray_y == 3 || n_tray_y == 4 || n_tray_y == 5 || n_tray_y == 6) mn_motor_no = M_RBT_SORTER_Z;

	if(n_tray_y == 7 || n_tray_y == 8 || n_tray_y == 9 ||
	   n_tray_y == 10 || n_tray_y == 11 || n_tray_y == 12 || n_tray_y == 13) mn_motor_no = M_RBT_SORTER_Y;
	
	if(n_tray_y == 14 || n_tray_y == 15 || n_tray_y == 16 ||
	   n_tray_y == 17 || n_tray_y == 18 || n_tray_y == 19 || n_tray_y == 20) mn_motor_no = M_RBT_SORTER_X;

	if(n_tray_y == 0) 
	{
		md_mot_start_pos = st_motor[mn_motor_no].md_pos[Z_SORT_SAFETY_UP];
	}
	else if(n_tray_y == 1 || n_tray_y == 2 || n_tray_y == 3 || n_tray_y == 4)
	{
		md_mot_start_pos = st_motor[mn_motor_no].md_pos[Z_SORT_BUFFER_PLACE1 + (n_tray_y -1)];	
	}
	else if(n_tray_y == 5) 
	{
		md_mot_start_pos = st_motor[mn_motor_no].md_pos[Z_SORT_VISION_PLACE];
	}
	else if(n_tray_y == 6) 
	{
		md_mot_start_pos = st_motor[mn_motor_no].md_pos[Z_SORT_VISION_PLACE2];
	}
	else if(n_tray_y == 7) 
	{
		md_mot_start_pos = st_motor[mn_motor_no].md_pos[Y_SORT_SAFETY];
	}
	else if(n_tray_y == 8 || n_tray_y == 9 || n_tray_y == 10 || n_tray_y == 11)
	{
		md_mot_start_pos = st_motor[mn_motor_no].md_pos[Y_SORT_WORK_PICK1 + (n_tray_y - 8)];	
	}
	else if(n_tray_y == 12) 
	{
		md_mot_start_pos = st_motor[mn_motor_no].md_pos[Y_SORT_VISION_BUFFER];
	}
	else if(n_tray_y == 13) 
	{
		md_mot_start_pos = st_motor[mn_motor_no].md_pos[Y_SORT_VISION_BUFFER2 - 4];
	}
	else if(n_tray_y == 14) 
	{
		md_mot_start_pos = st_motor[mn_motor_no].md_pos[X_SORT_SAFETY];
	}
	else if(n_tray_y == 15 || n_tray_y == 16 || n_tray_y == 17 || n_tray_y == 18)
	{
		md_mot_start_pos = st_motor[mn_motor_no].md_pos[X_SORT_WORK_PICK1 + (n_tray_y -15)];	
	}
	else if(n_tray_y == 19) 
	{
		md_mot_start_pos = st_motor[mn_motor_no].md_pos[X_SORT_VISION_BUFFER];
	}	
	else if(n_tray_y == 20) 
	{
		md_mot_start_pos = st_motor[mn_motor_no].md_pos[X_SORT_VISION_BUFFER2 - 2];
	}	

	SetTimer(TMR_SORTER_UNLOAD_MOT_CMD, 10, NULL);		// 모터 동작 제어 타이머
	// **************************************************************************		
	
}

void CDialog_Manual_sorter_unload::OnSite_RadioButton_Set()
{
...
void CDialog_Manual_sorter_unload::OnSite_IO_Status_Read()
{
	if (FAS_IO.get_in_bit(st_io.i_uld_stacker1_rail_fwd_chk, IO_ON) == IO_ON && 
		FAS_IO.get_in_bit(st_io.i_uld_stacker1_rail_bwd_chk, IO_OFF) == IO_OFF)		mn_io_clamp1_onoff = TRUE;
	else																			mn_io_clamp1_onoff = FALSE;

	if (FAS_IO.get_in_bit(st_io.i_uld_stacker2_rail_fwd_chk, IO_ON) == IO_ON && 
		FAS_IO.get_in_bit(st_io.i_uld_stacker2_rail_bwd_chk, IO_OFF) == IO_OFF)		mn_io_clamp2_onoff = TRUE;
	else																			mn_io_clamp2_onoff = FALSE;

	if (FAS_IO.get_in_bit(st_io.i_uld_stacker2_tray_clamp_on_chk, IO_ON) == IO_ON &&
		FAS_IO.get_in_bit(st_io.i_uld_stacker2_tray_clamp_off_chk, IO_OFF) == IO_OFF)	mn_io_guide2_onoff = TRUE;
	else																			mn_io_guide2_onoff = FALSE;

}
void CDialog_Manual_sorter_unload::OnSite_IO_Button_Status()
{
	// **************************************************************************
...
void CDialog_Manual_sorter_unload::OnBtnClamp1Onoff() 
{
	// TODO: Add your control notification handler code here
	int nRet_1;
	...
	{
		FAS_IO.set_out_bit(st_io.o_uld_stacker1_rail_fwd_onoff, IO_OFF);
		FAS_IO.set_out_bit(st_io.o_uld_stacker1_rail_bwd_onoff, IO_ON);
		SetDlgItemText(IDC_BTN_CLAMP1_ONOFF, "On");
	}
	else
	{
		FAS_IO.set_out_bit(st_io.o_uld_stacker1_rail_fwd_onoff, IO_ON);
		FAS_IO.set_out_bit(st_io.o_uld_stacker1_rail_bwd_onoff, IO_OFF);
		SetDlgItemText(IDC_BTN_CLAMP1_ONOFF, "Off");
	}	
}

void CDialog_Manual_sorter_unload::OnBtnClamp2Onoff() 
{
	// TODO: Add your control notification handler code here
	int nRet_1;
	...
	{
		FAS_IO.set_out_bit(st_io.o_uld_stacker2_rail_fwd_onoff, IO_OFF);
		FAS_IO.set_out_bit(st_io.o_uld_stacker2_rail_bwd_onoff, IO_ON);
...
void CDialog_Manual_sorter_unload::OnBtnGuide2Onff() 
{
	// TODO: Add your control notification handler code here
	int nRet_1;
...
void CDialog_Manual_sorter_unload::OnTimer(UINT nIDEvent) 
{
...
//	int nRet[5];
	...
	if (nIDEvent == TMR_SORTER_UNLOAD_MOT_CMD) 
	{
		// **************************************************************************
		...
		{
			KillTimer(TMR_SORTER_UNLOAD_MOT_CMD) ;  // 타이머 삭제
			mn_cmd_no = 0;				// 실행 명령 번호 저장 변수 초기화
	...
	else if (nIDEvent == TMR_SORTERUNLOAD_IO_MONITOR_CMD) 
	{
		OnSite_IO_Status_Read();
	}
	else if (nIDEvent == TMR_SORTER_STEPMOT_CMD) 
	{
		Run_Init();
		if (st_handler.mn_init_state[INIT_RBT_SORTER] == CTL_YES)
		{
			KillTimer(TMR_SORTER_STEPMOT_CMD);
			OnSite_Controls_Enable(TRUE);  // 모든 컨트롤 화면 출력 제어 함수			
		}
	}
	...
	else if (nIDEvent == TMR_UNLOAD_TRAY_MOT_CMD) 
	{
		mlp_AxisNum[0] = M_RBT_UNLOAD_X;        //현재의 IndexNum에서 사용하는 모터 실제 번호를 가진다 
		mlp_AxisNum[1] = M_RBT_UNLOAD_Y;  

		n_check = OnSite_Move_Excution1();			// 모터 이동 명령 처리 함수
		if (n_check == CTLBD_RET_GOOD)   
		{
			KillTimer(TMR_UNLOAD_TRAY_MOT_CMD) ;  // 타이머 삭제
...
void CDialog_Manual_sorter_unload::Set_PickerUpDown(int OnOff, int PickerInfo[D_INFOSIZE][PICKER_NUM])
{
	int i;
...
int CDialog_Manual_sorter_unload::Get_PickerUpDown(int OnOff, int PickerInfo[D_INFOSIZE][PICKER_NUM])
{
...
void CDialog_Manual_sorter_unload::Set_FingerOnOff(int OnOff, int FingerInfo[D_INFOSIZE][PICKER_NUM])
{
	int i;
...
int CDialog_Manual_sorter_unload::Get_FingerOnOff(int OnOff, int FingerInfo[D_INFOSIZE][PICKER_NUM])
{
	int i, FuncRet = RET_PROCEED;
	int RetVal[PICKER_NUM] = {0,};
	char jamcode[10] = {0,};
		...
		if(OnOff == IO_ON) //Finger ON (place라면..)
		{
			if(FingerInfo[D_EXIST][i] == YES) //동작 시킨 핑거면!
			{
				...
				else if(b_FingerFlag[i] == TRUE)
				{
					...
					if(l_FingerWaitTime[i][2] > st_wait.n_on_wait_time[53])
					{
						RetVal[i] = YES;
					}
				}
				else
				{
					...
					if(l_FingerWaitTime[i][2] > st_wait.n_limit_wait_time[53])
					{
...
void CDialog_Manual_sorter_unload::Run_Init()
{
...
BOOL CDialog_Manual_sorter_unload::PreTranslateMessage(MSG* pMsg) 
{
...
void CDialog_Manual_sorter_unload::OnBtnStop() 
{
	mn_stop_req = TRUE;	
}

int CDialog_Manual_sorter_unload::OnSite_Move_Excution()
{
...
int CDialog_Manual_sorter_unload::OnSite_Move_Excution1()
{
	int nFuncRet = CTLBD_RET_PROCEED;
	int nRet_1 = CTLBD_RET_PROCEED, n_chk, n_chk1;	
	double dSpeedRatio[3] = {st_basic.nRunSpeed,st_basic.nRunSpeed,st_basic.nRunSpeed};
	...
	case 0 :
		...
		// 모터 특정 위치로 이동시킨다. (이동위치 세팅~)                         
		// **********************************************************************
		n_chk = CTL_Lib.Linear_Move(UNLOAD_MOT_MAP, 2, mlp_AxisNum, dUldAbsPos, dSpeedRatio);
		if (n_chk == CTLBD_RET_GOOD)
		{
			mn_move_step = 0;
			nFuncRet = CTLBD_RET_GOOD;
...
//		break;
	...
	case 100 :
		...
		// 특정 축 긴급 정지 이벤트로 모터 정지한다                              
		// **********************************************************************
		n_chk = COMI.Set_MotStop(0, M_RBT_UNLOAD_X);
		n_chk1 = COMI.Set_MotStop(0, M_RBT_UNLOAD_Y);
		
		if (n_chk == TRUE && n_chk1 == TRUE)
			mn_move_step = 110;
		else 
		{
			if (st_handler.cwnd_list != NULL)  // 리스트 바 화면 존재
			{
				st_other.str_abnormal_msg = _T("[UNLOAD MODULE SITE] Though canceled transfer of motor, error happened.");
				st_handler.cwnd_list->PostMessage(WM_LIST_DATA, 0, ABNORMAL_MSG);  // 동작 실패 출력 요청
			}
			mn_move_step = 120;
		}
	...
	case 110 :
		if (st_handler.cwnd_list != NULL)  // 리스트 바 화면 존재
		{
			st_other.str_op_msg = _T("[UNLOAD MODULE SITE] Motor shut down during transfer.");
			st_handler.cwnd_list->PostMessage(WM_LIST_DATA, 0, NORMAL_MSG);  // 동작 완료 출력 요청
		}
	...
	case 120 :
		if (st_handler.cwnd_list != NULL)  // 리스트 바 화면 존재
		{
			st_other.str_abnormal_msg = _T("[UNLOAD MODULE SITE] Transfer command of motor failed.");
			st_handler.cwnd_list->PostMessage(WM_LIST_DATA, 0, ABNORMAL_MSG);  // 동작 실패 출력 요청
		}
...
void CDialog_Manual_sorter_unload::OnBtnHome() 
{
	CString strTmp;
	int nMotorPos = 0;
	...
	if(nMotorPos == 0)
	{
		mn_motor_no = M_RBT_UNLOAD_X;
	}
	else if(nMotorPos == 1)
	{
		mn_motor_no = M_RBT_UNLOAD_Y;
	}
	else if(nMotorPos == 2)
	{
		mn_motor_no = M_RBT_UNLOAD_Z;
	}
	else if(nMotorPos == 3)
	{
		mn_motor_no = M_RBT_SORTER_Z;
	...
	{
		mn_motor_no = M_RBT_SORTER_X;
	}
	else if(nMotorPos == 5)
	{
		mn_motor_no = M_RBT_SORTER_Y;
	}
	else if(nMotorPos == 6)
	{
		mn_motor_no = M_ULDM_STACKER_1;
	}
	else if(nMotorPos == 7)
	{
		mn_motor_no = M_ULDM_STACKER_2;
	}
	else if(nMotorPos == 8)
	{
		mn_motor_no = M_ULDM_STACKER_MOVE;
	}
	else
	{
		mn_motor_no = M_WORK_BUFFER_1 + nMotorPos - 9;
	}	
	...
	SetTimer(TMR_SORTER_UNLOAD_MOT_CMD, 100, NULL);			// 모터 동작 제어 타이머	
}

int CDialog_Manual_sorter_unload::OnSite_Homing_Excution()
{
...
void CDialog_Manual_sorter_unload::OnBtnGoStepRbtMove() 
{
	// TODO: Add your control notification handler code here
	CDialog_Message msg_dlg;
	int mn_response;
	...
	SetTimer(TMR_SORTER_STEPMOT_CMD, 100, NULL);			// 모터 동작 제어 타이머	
		
}

/////////////////////////////////////////// 20121006
void CDialog_Manual_sorter_unload::OnCellClick(WPARAM wParam, LPARAM lParam)
{
	...
	{
		m_n_axis = M_WORK_BUFFER_1 + nMotorPos - 6;
	}	
...
LONG CDialog_Manual_sorter_unload::OnUserLButtonDown(WPARAM wParam, LPARAM lParam)
{
	CDialog_Message msg_dlg;
	CString strTmp;
	int		nRet;
...
//			m_d_mot_goto_pos	= dmot_curr_pos - m_d_data;
			...
			SetTimer(TMR_SORTER_UNLOAD_MOT_CMD, 10, NULL);
		}
	}
...
LONG CDialog_Manual_sorter_unload::OnUserLButtonUp(WPARAM wParam, LPARAM lParam)
{	
	CDialog_Message msg_dlg;
	CString strTmp;
	int		nRet;
...
void CDialog_Manual_sorter_unload::OnButtonControl(BOOL b_flag)
{
	
}

void CDialog_Manual_sorter_unload::MotorMoveStepClear()
{
 	mn_stop_req = FALSE;	// ESTOP 요청 플래그 초기화
...
void CDialog_Manual_sorter_unload::MotorStatusCheck()
{
	m_grid_motor_info = (TSpread*)GetDlgItem(IDC_CUSTOM_MOTOR_INFO);
...
void CDialog_Manual_sorter_unload::MotorPositionCheck()
{
...
void CDialog_Manual_sorter_unload::OnSelchangeCmbMotorPos() 
{
	// TODO: Add your control notification handler code here
	int nMotorPos = m_cmb_motor_pos.GetCurSel();
...
void CDialog_Manual_sorter_unload::OnBtnGoStepRbtMove2() 
{
	// TODO: Add your control notification handler code here
	
}

void CDialog_Manual_sorter_unload::OnBtnGoBufferPos() 
{
	// TODO: Add your control notification handler code here
	int n_tray_y;
	int mn_FirstPicker_Y_Pos = 0;
	...
	md_mot_start_pos = st_motor[mn_motor_no].md_pos[WORK_ULDMODULE];
...
void CDialog_Manual_sorter_unload::OnBtnGoUnloaderPos() 
{
	// TODO: Add your control notification handler code here
	int n_tray_y;
	int mn_FirstPicker_Y_Pos = 0;
	...
	n_tray_y = m_cmb_unloader_pos.GetCurSel();
	...
	if(n_tray_y == 14 || n_tray_y == 15 || n_tray_y == 16 || n_tray_y == 17 ||
	   ...
	   n_tray_y == 26 || n_tray_y == 27 || n_tray_y == 28 || n_tray_y == 29 ||
	   n_tray_y == 30 || n_tray_y == 31 || n_tray_y == 32 || n_tray_y == 33 ||
	   n_tray_y == 34 || n_tray_y == 35)
	{
	}
	else
	{
...
//	mn_motor_no = M_RBT_UNLOAD_X;
	if(n_tray_y == 0 || n_tray_y == 1 || n_tray_y == 2 ||
	   n_tray_y == 3 || n_tray_y == 4 || n_tray_y == 5 || n_tray_y == 6) mn_motor_no = M_RBT_UNLOAD_X;

	if(n_tray_y == 7 || n_tray_y == 8 || n_tray_y == 9 ||
	   n_tray_y == 10 || n_tray_y == 11 || n_tray_y == 12 || n_tray_y == 13) mn_motor_no = M_RBT_UNLOAD_Y;

	if(n_tray_y == 14 || n_tray_y == 15 || n_tray_y == 16 ||
	   n_tray_y == 17 || n_tray_y == 18 || n_tray_y == 19 || n_tray_y == 20) mn_motor_no = M_RBT_UNLOAD_Z;


	if(n_tray_y == 21 || n_tray_y == 22 || n_tray_y == 23 || n_tray_y == 24 ||
		n_tray_y == 25 || n_tray_y == 26) mn_motor_no = M_ULDM_STACKER_1;
	
	if(n_tray_y == 27 || n_tray_y == 28 || n_tray_y == 29 || n_tray_y == 30 ||
		n_tray_y == 31 || n_tray_y == 32) mn_motor_no = M_ULDM_STACKER_2;
	
	if(n_tray_y == 33 || n_tray_y == 34 || n_tray_y == 35) mn_motor_no = M_ULDM_STACKER_MOVE;
...
void CDialog_Manual_sorter_unload::OnBtnCylinderSol2() 
{
	// TODO: Add your control notification handler code here
	if (m_btn_cylinder_sol2.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_uld_stacker1_up_onoff, IO_ON);
		FAS_IO.set_out_bit(st_io.o_uld_stacker1_dn_onoff, IO_OFF);
		mn_cylinder2[0] = TRUE;
	}
	else
	{
