...
public:
	...
	int SafetyAreaStep;
	int mn_io_board_initial_flag; //i/O 보드가 초기호가 완료되었는지 확인하는 플레그 

	bool bCylinderFlag[MAX_IO];
	long lCylinderTime[MAX_IO][3];

	long ml_delaysafety[3];
	int	n_retry;
...
//	int Check_IO_Port(int n_io, int n_time = 0, int n_flag = CTL_ON);
	...
	int Set_IO_Multi_Action(int nSite, int nModuleNo, int nOffset, int *npBit, int *npOnOff, int nBitCount, int n_onoff);	
	int Get_IO_Multi_ActionCheck(int nSite, int nModuleNo, int nOffset, int *npBit, int *npOnOff, int nBitCount, int nWaitTime, int nLimitTime);
	...
	int Reset_Safety_Area_OnOff(int nOnOff);//muting
