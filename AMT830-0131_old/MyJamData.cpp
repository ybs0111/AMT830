...
void CMyJamData::On_Alarm_Display()
{
	...
	CTime alarm_occured_time;
 	...
 	/* ************************************************************************** */
 	if (st_work.mn_run_status == CTL_dJAM || st_work.mn_run_status == CTL_dWARNING || st_work.mn_run_status == CTL_dLOTEND)  // 알람 발생
 	{	
...
// 		}
 		...
 		if (st_handler.mn_alarm_screen != TRUE)  // 알람 화면 미출력
 		{
 			...
 			if (alarm.mstr_code != "")
 			{
				st_handler.mstr_last_alarm = alarm.mstr_code;		// 마지막 알람을 저장한다.
 				/* ************************************************************************** */
...
// 					n_chk_time = alarm.stl_cur_alarm_time - alarm.stl_prev_alarm_time; 
					...
					n_chk_time = alarm.stl_cur_alarm_time - alarm.stl_alarm_start_time; 
 					...
 					if (n_chk_time < 5 * 60 * 1000/* || alarm.stl_alarm_start_time == 0*/)//현재 basic에 50으로 세팅되어 있음						// Basic에서 설정할수 있게 변경함. 2K9/11/19/ViboX
 					{
			...
			if (st_alarm.mstr_cur_state == "1" && n_data_save ==  CTL_YES)			// Count하는 Alarm이면... 2K9/08/28/ViboX
			{
				for (i = 0; i < 2000; i++)
				{
