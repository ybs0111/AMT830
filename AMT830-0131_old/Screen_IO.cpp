...
// 			if (data == IO_ON)
// 			{
// 				GridData(IDC_CUSTOM_IO, i+4, 4, "ON");
// 				GridFont(IDC_CUSTOM_IO, i+4, 4, 18);
// 				GridColor(IDC_CUSTOM_IO, i+4, 4, RGB(255, 128, 64), WHITE_C);
// 			}
// 			else
// 			{
// 				GridData(IDC_CUSTOM_IO, i+4, 4, "OFF");
// 				GridFont(IDC_CUSTOM_IO, i+4, 4, 18);
// 				GridColor(IDC_CUSTOM_IO, i+4, 4, RGB(128, 64, 0), BLACK_C);
// 			}
// 		}
// 	}
}
...
void CScreen_IO::OnDgtRetryWait() 
{
	...
	m_dgt_retry_wait.GetWindowRect(&r);
...
// 		Grid_Map->GetValue(1, lpcc->Row, chvalue);
// 		strTmp.Format("%s", chvalue);
// 		
// 		if(strTmp == "-") return;
// 		
// 		io = atoi(strTmp);
// 		
// 		Grid_Map->GetValue(5, lpcc->Row, chvalue);
// 		strTmp.Format("%s", chvalue);
// 		mstr_io_name = strTmp;
...
// 		{
// 			st_msg.mstr_fallacy_msg = _T("Door가 열려있으면, IO는 동작하지 않습니다.");
// 			if (st_handler.mn_language == LANGUAGE_ENGLISH)	st_msg.mstr_fallacy_msg = _T("If the Open Door, IO does not work");
// 			
...
// 				GridData(IDC_CUSTOM_IO, i+4, 4, "ON");
// 				GridFont(IDC_CUSTOM_IO, i+4, 4, 18);
// //				GridColor(IDC_CUSTOM_IO, i+4, 4, RGB(st_work.n_grid_r[0][0], st_work.n_grid_g[0][0], st_work.n_grid_b[0][0]), RGB(st_work.n_text_r[0][0], st_work.n_text_g[0][0], st_work.n_text_b[0][0]));
// 				GridColor(IDC_CUSTOM_IO, i+4, 4, RGB(0, 128, 0), WHITE_C);
// 			}
// 			else
// 			{
// 				GridData(IDC_CUSTOM_IO, i+4, 4, "OFF");
// 				GridFont(IDC_CUSTOM_IO, i+4, 4, 18);
// //				GridColor(IDC_CUSTOM_IO, i+4, 4, RGB(st_work.n_grid_r[0][1], st_work.n_grid_g[0][1], st_work.n_grid_b[0][1]), RGB(st_work.n_text_r[0][1], st_work.n_text_g[0][1], st_work.n_text_b[0][1]));
// 				GridColor(IDC_CUSTOM_IO, i+4, 4, RGB(128, 255, 128), BLACK_L);
// 			}
// 		}
// 	}
}
...
//	OnIO_Out_Port_Data_Display(mn_site);
	...
	case 0:
		...
		out_data = FAS_IO.Get_Out_Word(port);
		
		for (i = 0; i < 16; i++)
		{
			if (mn_output_info[i] == CTL_YES)
			{
				nAction = FAS_IO.get_out_bit(port + i, IO_OFF);
				FAS_IO.set_out_bit(port + i, !nAction);					
			}
		}
	...
	case 200:
		...
		for (i = 0; i < 16; i++)
		{
			if (mn_output_info[i] == CTL_YES)
			{
				nAction = FAS_IO.get_out_bit(port + i, IO_ON);
				FAS_IO.set_out_bit(port + i, !nAction);	
			}
		}
	...
	case 400:
		...
		if (st_handler.cwnd_list != NULL)  // 리스트 바 화면 존재
		{
			//				st_msg.mstr_normal_msg = _T(str_temp + "th repeat action.");
	...
	case 500:
		//			st_msg.mstr_normal_msg = _T("[Repeat action] The action was completed.!");
	...
	case 1000:
		//			st_msg.mstr_abnormal_msg = _T("A Repeat action became the cancel.");
