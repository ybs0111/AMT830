...
{
	int i;
	memset(st_client[m_n_address].ch_rev, 0x00, 8192);
	// 데이터 읽기
	for (i = 0;i < 200000000; i++);

	st_client[m_n_address].n_rev_length = Receive(st_client[m_n_address].ch_rev, 8192);


//	if(st_client[m_n_address].n_rev_length != SOCKET_ERROR)
//	{
...
//		}
//		for (i = 0;i < 2000000; i++);
//		::PostMessage(st_handler.hWnd, WM_CLIENT_MSG_1, CLIENT_REV, m_n_address);	// 메인 프레임으로 메시지 전송하여 메인 화면으로 전환 
//	}
//	else
//	{
//	}
	...
	::SendMessage(st_handler.hWnd, WM_CLIENT_MSG, CLIENT_REV, m_n_address);	// 메인 프레임으로 메시지 전송하여 메인 화면으로 전환 
...
void CClientSocket::OnClose(int nErrorCode) 
{
	...
	::PostMessage(st_handler.hWnd, WM_CLIENT_MSG_1, CLIENT_CLOSE, m_n_address);	// 메인 프레임으로 메시지 전송하여 메인 화면으로 전환 
