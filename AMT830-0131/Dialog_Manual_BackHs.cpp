// Dialog_Manual_BackHs.cpp : implementation file
//
...
CDialog_Manual_BackHs::CDialog_Manual_BackHs(CWnd* pParent /*=NULL*/)
	: CDialog(CDialog_Manual_BackHs::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDialog_Manual_BackHs)
	//}}AFX_DATA_INIT
	m_n_move_mode = 0;
...
void CDialog_Manual_BackHs::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDialog_Manual_BackHs)
	...
	DDX_Control(pDX, IDC_CMB_B_HS_POS, m_cmb_b_hs_pos);
	...
	DDX_Control(pDX, IDC_BTN_BUFFER_BACKHS_SOL1, m_btn_buffer_backhs_sol1);
	DDX_Control(pDX, IDC_BTN_BUFFER_BACKHS_SOL2, m_btn_buffer_backhs_sol2);
	DDX_Control(pDX, IDC_BTN_BUFFER_BACKHS_SOL3, m_btn_buffer_backhs_sol3);
	DDX_Control(pDX, IDC_BTN_BUFFER_BACKHS_SOL4, m_btn_buffer_backhs_sol4);
	DDX_Control(pDX, IDC_BTN_GO_B_HS_POS, m_btn_go_b_hs_pos);

	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CDialog_Manual_BackHs, CDialog)
	//{{AFX_MSG_MAP(CDialog_Manual_BackHs)
	...
	ON_BN_CLICKED(IDC_BTN_BUFFER_BACKHS_SOL1, OnBtnBufferBackhsSol1)
	ON_BN_CLICKED(IDC_BTN_BUFFER_BACKHS_SOL2, OnBtnBufferBackhsSol2)
	ON_BN_CLICKED(IDC_BTN_BUFFER_BACKHS_SOL3, OnBtnBufferBackhsSol3)
	ON_BN_CLICKED(IDC_BTN_BUFFER_BACKHS_SOL4, OnBtnBufferBackhsSol4)
	ON_BN_CLICKED(IDC_BTN_GO_B_HS_POS, OnBtnGoBHsPos)
	//}}AFX_MSG_MAP
/////////////////////////////20121006
...
/////////////////////////////////////////////////////////////////////////////
// CDialog_Manual_BackHs message handlers
BOOL CDialog_Manual_BackHs::OnInitDialog() 
{
...
////////////////////////////////////////////////////
	...
	if(nMotorPos == 0)
	{
		m_n_axis = M_HS_B_RBT_Y;
	}
	else if(nMotorPos == 1)
	{
		m_n_axis = M_HS_B_RBT_Z;
	}
	else if(nMotorPos == 2)
	{
		m_n_axis = M_HS_B_STACKER_1;
	}
	else if(nMotorPos == 3)
	{
		m_n_axis = M_HS_B_STACKER_2;
	}
	else if(nMotorPos == 4)
	{
		m_n_axis = M_HS_B_STACKER_MOVE;
	}
	else
	{
	...
	SetTimer(TMR_BACKHS_IO_MONITOR_CMD, 50, NULL);		// IO 정보 읽기 타이머 생성
	return TRUE;  // return TRUE unless you set the focus to a control
	              // EXCEPTION: OCX Property Pages should return FALSE
}

//////////////////////////////////////////////////// 20121006
void CDialog_Manual_BackHs::Init_Grid_Info()
{
	int   i, j;
	CString str_tmp;
...
void CDialog_Manual_BackHs::Init_GroupBox_Set()
{
...
void CDialog_Manual_BackHs::Init_Label()
{
	int i;
	CString str_line;
	...
	for (i = 0; i < 9; i++)
	{
		if(i == 0)
		{
			str_line.Format("%02d_BACKHS_Y", i + 1);
		}
		else if(i == 1)
		{
			str_line.Format("%02d_BACKHS_Z", i + 1);
		}
		else if(i == 2)
		{
	...
	m_cmb_b_hs_pos.SetFont(m_p_font);
	m_cmb_b_hs_pos.PostMessage(CB_SETITEMHEIGHT, (WPARAM)-1, (LPARAM) 20);
	...
	for (i = 0; i < 29; i++)
	{
		...
		m_cmb_b_hs_pos.AddString(str_line);
	}

	m_cmb_b_hs_pos.SetCurSel(0);
...
void CDialog_Manual_BackHs::Init_Data()
{
 	int i;
	...
	for (i = 0; i < 2; i++)
	{
		mn_picker[i] = 0;
		mn_finger[i] = 0;
		mn_backhs_buffer[i] = 0;
		mn_backhs_buffer[i + 2] = 0;
	}

//////////////////////////////////////////////////// 20121006
	m_n_axis			= st_motor_info.n_part_axis[2][AXIS_NUM];
		
	m_str_axis_name = st_motor_info.str_part_axis_name[2][AXIS_NUM];
	m_n_minus_el	= st_motor_info.n_axis_minus_el[2][AXIS_NUM];
	m_n_plus_el		= st_motor_info.n_axis_plus_el[2][AXIS_NUM];
	m_n_home		= st_motor_info.n_axis_home[2][AXIS_NUM];
	m_n_sd			= st_motor_info.n_axis_sd[2][AXIS_NUM];
	m_n_direction	= st_motor_info.n_axis_direction[2][AXIS_NUM];
	m_n_account		= st_motor_info.n_axis_account[2][AXIS_NUM];
////////////////////////////////////////////////////	
}

void CDialog_Manual_BackHs::Init_Button()
{
	short	shBtnColor = 30;
	CString strTmp;
...
//	m_btn_right.SetIcon(IDI_UP);
	...
	m_btn_buffer_backhs_sol1.SetFont(m_p_font);
	m_btn_buffer_backhs_sol1.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
	m_btn_buffer_backhs_sol1.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
	m_btn_buffer_backhs_sol1.SetButtonColor(1);

	m_btn_buffer_backhs_sol2.SetFont(m_p_font);
	m_btn_buffer_backhs_sol2.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
	m_btn_buffer_backhs_sol2.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
	m_btn_buffer_backhs_sol2.SetButtonColor(1);

	m_btn_buffer_backhs_sol3.SetFont(m_p_font);
	m_btn_buffer_backhs_sol3.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
	m_btn_buffer_backhs_sol3.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
	m_btn_buffer_backhs_sol3.SetButtonColor(1);

	m_btn_buffer_backhs_sol4.SetFont(m_p_font);
	m_btn_buffer_backhs_sol4.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
	m_btn_buffer_backhs_sol4.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
	m_btn_buffer_backhs_sol4.SetButtonColor(1);


}

void CDialog_Manual_BackHs::OnBtnPickerSol1() 
{
	// TODO: Add your control notification handler code here
	if (m_btn_picker_sol1.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_hs_bwd_rbt_picker_picker_updn[0], IO_ON);
		mn_picker[0] = TRUE;
	}
	else
	{
		FAS_IO.set_out_bit(st_io.o_hs_bwd_rbt_picker_picker_updn[0], IO_OFF);
		mn_picker[0] = FALSE;
	}	
	OnSiteRbt_Button_Status();	
...
void CDialog_Manual_BackHs::OnBtnPickerSol2() 
{
	// TODO: Add your control notification handler code here
	if (m_btn_picker_sol2.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_hs_bwd_rbt_picker_picker_updn[1], IO_ON);
		mn_picker[1] = TRUE;
	}
	else
	{
		FAS_IO.set_out_bit(st_io.o_hs_bwd_rbt_picker_picker_updn[1], IO_OFF);
		mn_picker[1] = FALSE;
	}	
	OnSiteRbt_Button_Status();		
...
void CDialog_Manual_BackHs::OnBtnPickerSolAll() 
{
	// TODO: Add your control notification handler code here
	int i;
	if (m_btn_picker_sol_all.GetCheck() == 0)//1번 기준
	{
		...
		{
			FAS_IO.set_out_bit(st_io.o_hs_bwd_rbt_picker_picker_updn[i], IO_OFF); 
			mn_picker[i] = FALSE;
		}		
	}
	else
	{
		for (i = 0; i < PICKER_NUM; i++)
		{
			FAS_IO.set_out_bit(st_io.o_hs_bwd_rbt_picker_picker_updn[i], IO_ON); 
			mn_picker[i] = TRUE;
		}		
	}
...
void CDialog_Manual_BackHs::OnBtnFingerSol1() 
{
	// TODO: Add your control notification handler code here
	if (m_btn_finger_sol1.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_hs_bwd_rbt_glipper_onoff[0], IO_ON);
		mn_finger[0] = TRUE;
	}
	else
	{
		FAS_IO.set_out_bit(st_io.o_hs_bwd_rbt_glipper_onoff[0], IO_OFF);
		mn_finger[0] = FALSE;
	}	
	OnSiteRbt_Button_Status();				
}

void CDialog_Manual_BackHs::OnBtnFingerSol2() 
{
	// TODO: Add your control notification handler code here
	if (m_btn_finger_sol2.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_hs_bwd_rbt_glipper_onoff[1], IO_ON);
		mn_finger[1] = TRUE;
	}
	else
	{
		FAS_IO.set_out_bit(st_io.o_hs_bwd_rbt_glipper_onoff[1], IO_OFF);
		mn_finger[1] = FALSE;
	}	
	OnSiteRbt_Button_Status();		
...
void CDialog_Manual_BackHs::OnBtnFingerSolAll() 
{
	// TODO: Add your control notification handler code here
	int i;
	if (m_btn_finger_sol_all.GetCheck() == 0)//1번 기준
	{
		for (i = 0; i < PICKER_NUM; i++)
		{
			FAS_IO.set_out_bit(st_io.o_hs_bwd_rbt_glipper_onoff[i], IO_OFF); 
			mn_finger[i] = FALSE;
		}		
	}
	else
	{
		for (i = 0; i < PICKER_NUM; i++)
		{
			FAS_IO.set_out_bit(st_io.o_hs_bwd_rbt_glipper_onoff[i], IO_ON); 
			mn_finger[i] = TRUE;
		}		
	}
...
void CDialog_Manual_BackHs::OnBtnBufferBackhsSol1()
{
	// TODO: Add your control notification handler code here
	if (m_btn_buffer_backhs_sol1.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_buf1_hs_clamp_onoff, IO_ON);
		mn_backhs_buffer[0] = TRUE;
	}
	else
	{
		FAS_IO.set_out_bit(st_io.o_buf1_hs_clamp_onoff, IO_OFF);
		mn_backhs_buffer[0] = FALSE;
	}	
	OnSiteRbt_Button_Status();
	
}

void CDialog_Manual_BackHs::OnBtnBufferBackhsSol2()
{
	// TODO: Add your control notification handler code here
	if (m_btn_buffer_backhs_sol2.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_buf2_hs_clamp_onoff, IO_ON);
		mn_backhs_buffer[1] = TRUE;
	}
	else
	{
		FAS_IO.set_out_bit(st_io.o_buf2_hs_clamp_onoff, IO_OFF);
		mn_backhs_buffer[1] = FALSE;
	}	
	OnSiteRbt_Button_Status();	
	
}

void CDialog_Manual_BackHs::OnBtnBufferBackhsSol3()
{
	// TODO: Add your control notification handler code here
	if (m_btn_buffer_backhs_sol3.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_buf3_hs_clamp_onoff, IO_ON);
		mn_backhs_buffer[2] = TRUE;
	}
	else
	{
		FAS_IO.set_out_bit(st_io.o_buf3_hs_clamp_onoff, IO_OFF);
		mn_backhs_buffer[2] = FALSE;
	}	
	OnSiteRbt_Button_Status();	
	
}

void CDialog_Manual_BackHs::OnBtnBufferBackhsSol4()
{
	// TODO: Add your control notification handler code here
	if (m_btn_buffer_backhs_sol4.GetCheck() == 1)
	{
		FAS_IO.set_out_bit(st_io.o_buf4_hs_clamp_onoff, IO_ON);
		mn_backhs_buffer[3] = TRUE;
	}
	else
	{
		FAS_IO.set_out_bit(st_io.o_buf4_hs_clamp_onoff, IO_OFF);
		mn_backhs_buffer[3] = FALSE;
	}	
	OnSiteRbt_Button_Status();	
	
}

void CDialog_Manual_BackHs::OnRead_IO_Check()
{
	int i;
	...
	for(i=0; i<2; i++)
	{
		if(FAS_IO.get_in_bit(st_io.o_hs_bwd_rbt_picker_picker_updn[i], IO_ON) == IO_ON)
		{
			mn_picker[i] = TRUE;
		}
		else
	...
	for(i=0; i<2; i++)
	{
		if(FAS_IO.get_in_bit(st_io.o_hs_bwd_rbt_glipper_onoff[i], IO_ON) == IO_ON)
		{
			mn_finger[i] = TRUE;
		}
		else
	...
	for(i=0; i<4; i++)
	{
		...
		{
			mn_backhs_buffer[i] = TRUE;
		}
		else
		{
			mn_backhs_buffer[i] = FALSE;		
		}	
	}

}

void CDialog_Manual_BackHs::OnSiteRbt_Button_Status()//20121006
{
	short	shBtnColor = 30;
	...
	if (mn_backhs_buffer[0] < 0 || mn_backhs_buffer[0] > 1) mn_backhs_buffer[0] = TRUE;
	m_btn_buffer_backhs_sol1.SetCheck(mn_backhs_buffer[0]);
	if (mn_backhs_buffer[0] == TRUE)
	{
		m_btn_buffer_backhs_sol1.SetCheck(TRUE);
		SetDlgItemText(IDC_BTN_BUFFER_BACKHS_SOL1, "On");
		m_btn_buffer_backhs_sol1.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_buffer_backhs_sol1.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_buffer_backhs_sol1.SetButtonColor(1);
	...
	{
		m_btn_buffer_backhs_sol1.SetCheck(FALSE);
		SetDlgItemText(IDC_BTN_BUFFER_BACKHS_SOL1, "Off");
		m_btn_buffer_backhs_sol1.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_buffer_backhs_sol1.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_buffer_backhs_sol1.SetButtonColor(1);
	...
	if (mn_backhs_buffer[1] < 0 || mn_backhs_buffer[1] > 1) mn_backhs_buffer[1] = TRUE;
	m_btn_buffer_backhs_sol2.SetCheck(mn_backhs_buffer[1]);
	if (mn_backhs_buffer[1] == TRUE)
	{
		m_btn_buffer_backhs_sol2.SetCheck(TRUE);
		SetDlgItemText(IDC_BTN_BUFFER_BACKHS_SOL2, "On");
		m_btn_buffer_backhs_sol2.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_buffer_backhs_sol2.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_buffer_backhs_sol2.SetButtonColor(1);
	...
	{
		m_btn_buffer_backhs_sol2.SetCheck(FALSE);
		SetDlgItemText(IDC_BTN_BUFFER_BACKHS_SOL2, "Off");
		m_btn_buffer_backhs_sol2.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_buffer_backhs_sol2.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_buffer_backhs_sol2.SetButtonColor(1);
	...
	if (mn_backhs_buffer[2] < 0 || mn_backhs_buffer[2] > 1) mn_backhs_buffer[2] = TRUE;
	m_btn_buffer_backhs_sol3.SetCheck(mn_backhs_buffer[2]);
	if (mn_backhs_buffer[2] == TRUE)
	{
		m_btn_buffer_backhs_sol3.SetCheck(TRUE);
		SetDlgItemText(IDC_BTN_BUFFER_BACKHS_SOL3, "On");
		m_btn_buffer_backhs_sol3.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_buffer_backhs_sol3.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_buffer_backhs_sol3.SetButtonColor(1);
	...
	{
		m_btn_buffer_backhs_sol3.SetCheck(FALSE);
		SetDlgItemText(IDC_BTN_BUFFER_BACKHS_SOL3, "Off");
		m_btn_buffer_backhs_sol3.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_buffer_backhs_sol3.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_buffer_backhs_sol3.SetButtonColor(1);
	...
	if (mn_backhs_buffer[3] < 0 || mn_backhs_buffer[3] > 1) mn_backhs_buffer[3] = TRUE;
	m_btn_buffer_backhs_sol4.SetCheck(mn_backhs_buffer[3]);
	if (mn_backhs_buffer[3] == TRUE)
	{
		m_btn_buffer_backhs_sol4.SetCheck(TRUE);
		SetDlgItemText(IDC_BTN_BUFFER_BACKHS_SOL4, "On");
		m_btn_buffer_backhs_sol4.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_buffer_backhs_sol4.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_buffer_backhs_sol4.SetButtonColor(1);
	...
	{
		m_btn_buffer_backhs_sol4.SetCheck(FALSE);
		SetDlgItemText(IDC_BTN_BUFFER_BACKHS_SOL4, "Off");
		m_btn_buffer_backhs_sol4.SetIcon(IDI_GREEN_LED_ICON, IDI_RED_LED_ICON);
		m_btn_buffer_backhs_sol4.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
		m_btn_buffer_backhs_sol4.SetButtonColor(1);
...
void CDialog_Manual_BackHs::OnSite_Controls_Enable(BOOL iFlag)
{
	m_btn_go_b_hs_pos.EnableWindow(iFlag);
	...
	m_btn_buffer_backhs_sol1.EnableWindow(iFlag);
	m_btn_buffer_backhs_sol2.EnableWindow(iFlag);
	m_btn_buffer_backhs_sol3.EnableWindow(iFlag);
	m_btn_buffer_backhs_sol4.EnableWindow(iFlag);
}

void CDialog_Manual_BackHs::OnSite_Step_Clear()
{
...
void CDialog_Manual_BackHs::OnBtnGoTrayPos() 
{
	// TODO: Add your control notification handler code here
	int x, n_tray_y;
	...
	if (n_tray_y < 0)	
	{
		...
		{
			sprintf(st_msg.c_abnormal_msg,"[Backward Heat Sink RBT] can not move.");
	...
	////////////////////// 20121107
	nRet = COMI.Check_SingleMove(M_HS_B_RBT_Z, Z_LD_SAFETY_UP);
	...
	mn_motor_no = M_HS_B_RBT_Y;

	for(x=0 ; x<PICKER_NUM; x++) 
	{
		if(st_modulemap.nRighthsRobotPicker[D_EXIST][x] == DVC_NO && st_picker.n_heat_rightldrbt_enable[x] == YES)
		{
			mn_FirstPicker_Y_Pos = x;
			break;
		}
	}
	md_mot_start_pos = Func.XYRobot_MovePosValCheck(RIGHTSINK_SITE, 0, mn_FirstPicker_Y_Pos, n_tray_y); // 트레이에서 찾은 위치로 레일을 이동시킨다
	
	SetTimer(TMR_BACKHSMOT_CMD, 10, NULL);		// 모터 동작 제어 타이머
	// **************************************************************************		
}

void CDialog_Manual_BackHs::OnBtnGoBufferPos() 
{
	// TODO: Add your control notification handler code here
	int n_tray_y;
	int mn_FirstPicker_Y_Pos = 0;
	...
	md_mot_start_pos = st_motor[mn_motor_no].md_pos[WORK_LDRIGHTTHS];
	
	SetTimer(TMR_BACKHSMOT_CMD, 10, NULL);		// 모터 동작 제어 타이머
	// **************************************************************************			
}

void CDialog_Manual_BackHs::OnSite_RadioButton_Set()
{
...
void CDialog_Manual_BackHs::OnSite_IO_Status_Read()
{
	if (FAS_IO.get_in_bit(st_io.i_hs_bwd_stacker1_rail_fwd_chk, IO_ON) == IO_ON && 
		FAS_IO.get_in_bit(st_io.i_hs_bwd_stacker1_rail_bwd_chk, IO_OFF) == IO_OFF)		mn_io_clamp1_onoff = TRUE;
	else																			mn_io_clamp1_onoff = FALSE;

	if (FAS_IO.get_in_bit(st_io.i_hs_bwd_stacker2_rail_fwd_chk, IO_ON) == IO_ON && 
		FAS_IO.get_in_bit(st_io.i_hs_bwd_stacker2_rail_bwd_chk, IO_OFF) == IO_OFF)		mn_io_clamp2_onoff = TRUE;
	else																			mn_io_clamp2_onoff = FALSE;

	if (FAS_IO.get_in_bit(st_io.i_hs_bwd_stacker2_tray_clamp_on_chk, IO_ON) == IO_ON &&
		FAS_IO.get_in_bit(st_io.i_hs_bwd_stacker2_tray_clamp_off_chk, IO_OFF) == IO_OFF)	mn_io_guide2_onoff = TRUE;
	else																			mn_io_guide2_onoff = FALSE;

}

void CDialog_Manual_BackHs::OnSite_IO_Button_Status()
{
	// **************************************************************************
...
void CDialog_Manual_BackHs::OnBtnClamp1Onoff() 
{
	// TODO: Add your control notification handler code here
	int nRet_1;
	...
	{
		FAS_IO.set_out_bit(st_io.o_hs_bwd_stacker1_rail_fwd_onoff, IO_OFF);
		FAS_IO.set_out_bit(st_io.o_hs_bwd_stacker1_rail_bwd_onoff, IO_ON);
		SetDlgItemText(IDC_BTN_CLAMP1_ONOFF, "On");
	}
	else
	{
		FAS_IO.set_out_bit(st_io.o_hs_bwd_stacker1_rail_fwd_onoff, IO_ON);
		FAS_IO.set_out_bit(st_io.o_hs_bwd_stacker1_rail_bwd_onoff, IO_OFF);
		SetDlgItemText(IDC_BTN_CLAMP1_ONOFF, "Off");
	}
	
}

void CDialog_Manual_BackHs::OnBtnClamp2Onoff() 
{
	// TODO: Add your control notification handler code here
	int nRet_1;
	...
	{
		FAS_IO.set_out_bit(st_io.o_hs_bwd_stacker2_rail_fwd_onoff, IO_OFF);
		FAS_IO.set_out_bit(st_io.o_hs_bwd_stacker2_rail_bwd_onoff, IO_ON);
...
void CDialog_Manual_BackHs::OnBtnGuide2Onff() 
{
	// TODO: Add your control notification handler code here
	int nRet_1;
...
void CDialog_Manual_BackHs::OnTimer(UINT nIDEvent) 
{
...
//	int nRet[5];
	...
	if (nIDEvent == TMR_BACKHSMOT_CMD) 
	{
		...
		{
			KillTimer(TMR_BACKHSMOT_CMD) ;  // 타이머 삭제
			mn_cmd_no = 0;				// 실행 명령 번호 저장 변수 초기화
	...
	else if (nIDEvent == TMR_BACKHS_IO_MONITOR_CMD) 
	{
		OnSite_IO_Status_Read();
	}
	else if (nIDEvent == TMR_BACKHSSTEPMOT_CMD) 
	{
		Run_Init();
		if (st_handler.mn_init_state[INIT_RBT_RIGHT_SINK] == CTL_YES)
		{
			KillTimer(TMR_BACKHSSTEPMOT_CMD);
			OnSite_Controls_Enable(TRUE);  // 모든 컨트롤 화면 출력 제어 함수			
		}
	}
...
void CDialog_Manual_BackHs::Set_PickerUpDown(int OnOff, int PickerInfo[D_INFOSIZE][PICKER_NUM])
{
	int i;
...
int CDialog_Manual_BackHs::Get_PickerUpDown(int OnOff, int PickerInfo[D_INFOSIZE][PICKER_NUM])
{
	...
	for(i = 0 ; i < PICKER_NUM ; i++)
	{
		...
		else //Picker OFF (UP이라면..)
		{
			if(PickerInfo[D_EXIST][i] == YES) //동작 시킨 피커면!
			{
				if(b_PickerFlag[i] == FALSE && FAS_IO.get_in_bit(st_io.i_hs_bwd_picker_dn_chk[i], IO_OFF) == IO_OFF
					&& FAS_IO.Get_In_Bit(st_io.i_hs_bwd_picker_up_chk[i]) == IO_ON)
				{
					b_PickerFlag[i] = TRUE;
					l_PickerWaitTime[i][0] = GetCurrentTime();   
				}
				else if(b_PickerFlag[i] == TRUE && FAS_IO.get_in_bit(st_io.i_hs_bwd_picker_dn_chk[i], IO_OFF) == IO_OFF
					&& FAS_IO.Get_In_Bit(st_io.i_hs_bwd_picker_up_chk[i]) == IO_ON)
				{
					l_PickerWaitTime[i][1] = GetCurrentTime();  
					l_PickerWaitTime[i][2] = l_PickerWaitTime[i][1] - l_PickerWaitTime[i][0];
...
void CDialog_Manual_BackHs::Set_FingerOnOff(int OnOff, int FingerInfo[D_INFOSIZE][PICKER_NUM])
{
	int i;
...
int CDialog_Manual_BackHs::Get_FingerOnOff(int OnOff, int FingerInfo[D_INFOSIZE][PICKER_NUM])
{
	int i, FuncRet = RET_PROCEED;
	int RetVal[PICKER_NUM] = {0,};
	char jamcode[10] = {0,};
...
void CDialog_Manual_BackHs::Run_Init()
{
		...
		for (i = 0; i < PICKER_NUM; i++)
		{
			if(ActionPicker[D_EXIST][i] == DVC_YES && st_picker.n_heat_rightldrbt_enable[i] == YES)
			{
				if(FAS_IO.get_in_bit(st_io.i_hs_bwd_dvc_chk[i], IO_ON) == IO_OFF)
				{
					if(st_handler.cwnd_list != NULL)
					{
						sprintf(st_msg.c_abnormal_msg,"[Initialize Error] Backward Heat Sink Picker #1 Module On Check Error.");
						st_handler.cwnd_list->PostMessage(WM_LIST_DATA, 0, ABNORMAL_MSG);
					}
					nRet_1 = RET_ERROR;
					break;
...
BOOL CDialog_Manual_BackHs::PreTranslateMessage(MSG* pMsg) 
{
...
void CDialog_Manual_BackHs::OnBtnStop() 
{
	// TODO: Add your control notification handler code here
	mn_stop_req = TRUE;
}

int CDialog_Manual_BackHs::OnSite_Move_Excution()
{
	...
	case 10 :
		...
		if (n_chk == CTLBD_RET_GOOD)
		{
			if (st_handler.cwnd_list != NULL)  // 리스트 바 화면 존재
			{
				sprintf(st_msg.c_normal_msg, "[BACK HS SITE] Motor completed transfer.");
	...
	case 100 :
		...
		else 
		{
			if (st_handler.cwnd_list != NULL)  // 리스트 바 화면 존재
			{
				st_other.str_abnormal_msg = _T("[BACK HS SITE] Though canceled transfer of motor, error happened.");
				st_handler.cwnd_list->PostMessage(WM_LIST_DATA, 0, ABNORMAL_MSG);  // 동작 실패 출력 요청
			}
			mn_move_step = 120;
		}
	...
	case 110 :
		if (st_handler.cwnd_list != NULL)  // 리스트 바 화면 존재
		{
			st_other.str_op_msg = _T("[BACK HS SITE] Motor shut down during transfer.");
			st_handler.cwnd_list->PostMessage(WM_LIST_DATA, 0, NORMAL_MSG);  // 동작 완료 출력 요청
		}
	...
	case 120 :
		if (st_handler.cwnd_list != NULL)  // 리스트 바 화면 존재
		{
			st_other.str_abnormal_msg = _T("[BACK HS SITE] Transfer command of motor failed.");
			st_handler.cwnd_list->PostMessage(WM_LIST_DATA, 0, ABNORMAL_MSG);  // 동작 실패 출력 요청
		}
...
void CDialog_Manual_BackHs::OnBtnHome() 
{
	CString strTmp;
	int nMotorPos = 0;
	...
	{
		mn_motor_no = M_HS_B_RBT_Z;
	}
	else if(nMotorPos == 2)
	{
		mn_motor_no = M_HS_B_STACKER_1;
	}
	else if(nMotorPos == 3)
	{
		mn_motor_no = M_HS_B_STACKER_2;
	}
	else if(nMotorPos == 4)
	{
		mn_motor_no = M_HS_B_STACKER_MOVE;
	}
	else
	{
	...
	SetTimer(TMR_BACKHSMOT_CMD, 100, NULL);			// 모터 동작 제어 타이머	
}
...
int CDialog_Manual_BackHs::OnSite_Homing_Excution()
{
...
void CDialog_Manual_BackHs::OnBtnGoStepRbtMove() 
{
	// TODO: Add your control notification handler code here
	CDialog_Message msg_dlg;
	int mn_response;
	...
	SetTimer(TMR_BACKHSSTEPMOT_CMD, 100, NULL);			// 모터 동작 제어 타이머		
}

/////////////////////////////////////////// 20121006
void CDialog_Manual_BackHs::OnCellClick(WPARAM wParam, LPARAM lParam)
{
	...
	if (nMotorPos < 0)	
	{
		if (st_handler.cwnd_list != NULL)  // 리스트 바 화면 존재
		{
			sprintf(st_msg.c_abnormal_msg,"[Manual Back HS] 이동 축이 선택되지 않았습니다.");
			if(st_handler.mn_language == LANGUAGE_ENGLISH) 
			{
				sprintf(st_msg.c_abnormal_msg, "[Manual Back HS] is not selected Axis");
			}
...
LONG CDialog_Manual_BackHs::OnUserLButtonDown(WPARAM wParam, LPARAM lParam)
{
	CDialog_Message msg_dlg;
	CString strTmp;
	int		nRet;
...
//			m_d_mot_goto_pos	= dmot_curr_pos - m_d_data;
			...
			SetTimer(TMR_BACKHSMOT_CMD, 10, NULL);
		}
	}
...
LONG CDialog_Manual_BackHs::OnUserLButtonUp(WPARAM wParam, LPARAM lParam)
{	
	CDialog_Message msg_dlg;
	CString strTmp;
	int		nRet;
...
void CDialog_Manual_BackHs::OnButtonControl(BOOL b_flag)
{

}

void CDialog_Manual_BackHs::MotorMoveStepClear()
{
 	mn_stop_req = FALSE;	// ESTOP 요청 플래그 초기화
...
void CDialog_Manual_BackHs::MotorStatusCheck()
{
	m_grid_motor_info = (TSpread*)GetDlgItem(IDC_CUSTOM_MOTOR_INFO);
...
void CDialog_Manual_BackHs::MotorPositionCheck()
{
...
void CDialog_Manual_BackHs::OnSelchangeCmbMotorPos() 
{
	// TODO: Add your control notification handler code here
	int nMotorPos = m_cmb_motor_pos.GetCurSel();
...
void CDialog_Manual_BackHs::OnBtnGoBHsPos() 
{
	// TODO: Add your control notification handler code here
	int n_tray_y;
	int mn_FirstPicker_Y_Pos = 0;
	...
	n_tray_y = m_cmb_b_hs_pos.GetCurSel();
...
//	mn_motor_no = M_HS_B_RBT_Y;
	...
	if(n_tray_y == 0 || n_tray_y == 1 || n_tray_y == 2 || n_tray_y == 3 ||
	   n_tray_y == 4 || n_tray_y == 5 || n_tray_y == 6)	mn_motor_no = M_HS_B_RBT_Y;
	if(n_tray_y == 7 || n_tray_y == 8 || n_tray_y == 9 || n_tray_y == 10 ||
	   n_tray_y == 11 || n_tray_y == 12 || n_tray_y == 13) mn_motor_no = M_HS_B_RBT_Z;

	if(n_tray_y == 14 || n_tray_y == 15 || n_tray_y == 16 || n_tray_y == 17 ||
	   n_tray_y == 18 || n_tray_y == 19) mn_motor_no = M_HS_B_STACKER_1;

	if(n_tray_y == 20 || n_tray_y == 21 || n_tray_y == 22 || n_tray_y == 23 ||
	   n_tray_y == 24 || n_tray_y == 25) mn_motor_no = M_HS_B_STACKER_2;

	if(n_tray_y == 26 || n_tray_y == 27 || n_tray_y == 28) mn_motor_no = M_HS_B_STACKER_MOVE;
