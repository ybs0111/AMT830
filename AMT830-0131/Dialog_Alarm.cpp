...
void CDialog_Alarm::OnAlarm_Lable_Set()
{
	...
	m_msg_alarm_time.SetTextFont(mp_alarm_font);
...
void CDialog_Alarm::OnAlarm_Default_Data_Load()
{
	...
	{
		mstr_action_file = "JamAction.TXT";		// 알람 조치 사항 정보 로딩 파일명 설정 
	}
	else
	{
		mstr_action_file = "JamAction.TXT";			// 알람 조치 사항 정보 로딩 파일명 설정 
...
{	
	...
	if (st_handler.cwnd_list != NULL)  st_handler.cwnd_list->PostMessage(WM_LIST_DATA, 0, NORMAL_MSG);	FAS_IO.set_out_bit(st_io.o_alarm_reset_switch_lamp, IO_OFF);	// Reset Switch에 Lamp를 Off 시킨다.
	Func.OnSet_IO_Port_Sound(IO_OFF);
	DestroyWindow();
}
