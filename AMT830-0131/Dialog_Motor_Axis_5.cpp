// Dialog_Motor_Axis_5.cpp : implementation file
//
...
#define AXIS_NUM			4
CDialog_Motor_Axis_5::CDialog_Motor_Axis_5(CWnd* pParent /*=NULL*/)
	: CDialog(CDialog_Motor_Axis_5::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDialog_Motor_Axis_5)
...
void CDialog_Motor_Axis_5::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDialog_Motor_Axis_5)
...
BEGIN_MESSAGE_MAP(CDialog_Motor_Axis_5, CDialog)
	//{{AFX_MSG_MAP(CDialog_Motor_Axis_5)
	ON_WM_TIMER()
	ON_WM_DESTROY()
...
/////////////////////////////////////////////////////////////////////////////
// CDialog_Motor_Axis_5 message handlers
/*
void CDialog_Motor_Axis_5::OnTimer(UINT nIDEvent) 
{
...
*/
BOOL CDialog_Motor_Axis_5::OnInitDialog() 
{
...
void CDialog_Motor_Axis_5::Init_Group()
{
...
void CDialog_Motor_Axis_5::Init_Button()
{
	short	shBtnColor = 30;
	CString strTmp;

	if (m_str_axis_name == "Unloader_Transfer_X")
	{
		m_btn_left.SetIcon(IDI_LEFT);
	}
///////////////////////// 20120716
	else if (m_str_axis_name == "Vision2_Z")// || m_str_axis_name == "Loader_Transfer_X")
	{
		m_btn_left.SetIcon(IDI_DN);
	}
	else if (m_str_axis_name == "S_Rbt_X")// || m_str_axis_name == "Loader_Transfer_X")
	{
		m_btn_left.SetIcon(IDI_LEFT);
	}
	else if (m_str_axis_name == "H_B_RBT_Z" || m_str_axis_name == "H_F_RBT_Z" || m_str_axis_name == "M_RBT_Z")// || m_str_axis_name == "Loader_Transfer_X")
	{
...
///////////////////////// 20120716
	...
	else if (m_str_axis_name == "H_B_RBT_Z" || m_str_axis_name == "H_F_RBT_Z" || m_str_axis_name == "M_RBT_Z")
	{
...
void CDialog_Motor_Axis_5::Init_Label()
{

}

void CDialog_Motor_Axis_5::Init_Radio()
{

}

void CDialog_Motor_Axis_5::Init_Digit()
{

}

void CDialog_Motor_Axis_5::Init_Grid_Pos()
{
	int   i, j;
	CString str_tmp;
...
void CDialog_Motor_Axis_5::Data_Apply()
{
	int i;
...
void CDialog_Motor_Axis_5::Data_Backup()
{
	int i;
...
int CDialog_Motor_Axis_5::Data_Comparison()
{
	int i;
...
void CDialog_Motor_Axis_5::Data_HistoryLog()
{
	int i;
	CString str_msg;
...
void CDialog_Motor_Axis_5::Data_Recovery()
{
	int i;
...
void CDialog_Motor_Axis_5::Data_Init()
{
	int i;
...
void CDialog_Motor_Axis_5::MotorStatusCheck()
{
...
void CDialog_Motor_Axis_5::MotorPositionCheck()
{
...
void CDialog_Motor_Axis_5::OnCellClick(WPARAM wParam, LPARAM lParam)
{
	...
	if(wParam == IDC_CUSTOM_MOTOR_POS)
	{
		...
		case 3:
			...
			///////////////////////////////////////// 20121114
			if (m_str_axis_name == "Vision2_Z")
			{
				dCurr1 = COMI.Get_MotCurrentPos(M_VISION1_Y);
				dCurr2 = COMI.Get_MotCurrentPos(M_VISION2_Y);

				if(dCurr1 <= st_motor[M_VISION1_Y].md_pos[Z_VISION_SAFETY] + st_motor[M_VISION1_Y].mn_allow &&
					dCurr2 >= st_motor[M_VISION2_Y].md_pos[Z_VISION_SAFETY] + st_motor[M_VISION2_Y].mn_allow) 
					{//Z축 1번, 2번이 Unloading일 때 
						st_msg.str_fallacy_msg = _T("Vision Buffer1, 2번 Y축 모두 Safety 위치입니다. 한 축을 이동 하세요");	
						if(st_handler.mn_language == LANGUAGE_ENGLISH)
						{
							st_msg.str_fallacy_msg = _T("Vision Buffer1, 2 times all the Y-axis position of Safety. Go to the axis");
						}
						
						n_response = msg_dlg.DoModal();
						return;	
					}
				else if(dCurr1 <= st_motor[M_VISION1_Y].md_pos[Z_VISION_LDMODULE] + st_motor[M_VISION1_Y].mn_allow &&
					dCurr2 >= st_motor[M_VISION2_Y].md_pos[Z_VISION_LDMODULE] + st_motor[M_VISION2_Y].mn_allow)
					{//Z축 1번, 2번이 Loading일 때 
						st_msg.str_fallacy_msg = _T("Vision Buffer1, 2번 Y축 모두 Module 위치입니다. 한 축을 이동 하세요");	
						if(st_handler.mn_language == LANGUAGE_ENGLISH)
						{
							st_msg.str_fallacy_msg = _T("Vision Buffer1, 2 times all the Y-axis position of Module. Go to the axis");
						}
						
						n_response = msg_dlg.DoModal();
						return;	
					}			
				else if(dCurr1 <= st_motor[M_VISION1_Y].md_pos[Z_VISION_LABEL] + st_motor[M_VISION1_Y].mn_allow &&
					dCurr2 >= st_motor[M_VISION2_Y].md_pos[Z_VISION_LABEL] + st_motor[M_VISION2_Y].mn_allow)
					{//Z축 1번, 2번이 Label일 때 
						st_msg.str_fallacy_msg = _T("Vision Buffer1, 2번 Y축 모두 Label 위치입니다. 한 축을 이동 하세요");	
						if(st_handler.mn_language == LANGUAGE_ENGLISH)
						{
							st_msg.str_fallacy_msg = _T("Vision Buffer1, 2 times all the Y-axis position of Label. Go to the axis");
						}
						
						n_response = msg_dlg.DoModal();
						return;	
					}
				else if(dCurr1 <= st_motor[M_VISION1_Y].md_pos[Z_VISION_CHK] + st_motor[M_VISION1_Y].mn_allow &&
					dCurr2 >= st_motor[M_VISION2_Y].md_pos[Z_VISION_CHK] + st_motor[M_VISION2_Y].mn_allow) 
					{//Z축 1번, 2번이 Vision일 때 
						st_msg.str_fallacy_msg = _T("Vision Buffer1, 2번 Y축 모두 Vision Check 위치입니다. 한 축을 이동 하세요");	
						if(st_handler.mn_language == LANGUAGE_ENGLISH)
						{
							st_msg.str_fallacy_msg = _T("Vision Buffer1, 2 times all the Y-axis position of Vision Check. Go to the axis");
						}
						
						n_response = msg_dlg.DoModal();
						return;	
					}
				else if(dCurr1 >= st_motor[M_VISION1_Y].md_pos[Z_VISION_ULDMODULE] + st_motor[M_VISION1_Y].mn_allow &&
					dCurr2 <= st_motor[M_VISION2_Y].md_pos[Z_VISION_ULDMODULE] + st_motor[M_VISION2_Y].mn_allow) 
					{//Z축 1번, 2번이 Unloading일 때 
						st_msg.str_fallacy_msg = _T("Vision Buffer1, 2번 Y축 모두 Unload 위치입니다. 한 축을 이동 하세요");	
						if(st_handler.mn_language == LANGUAGE_ENGLISH)
						{
							st_msg.str_fallacy_msg = _T("Vision Buffer1, 2 times all the Y-axis position of Unload. Go to the axis");
						}
...
void CDialog_Motor_Axis_5::MotorMoveStepClear()
{
...
void CDialog_Motor_Axis_5::MotorCompletion(int nrow, int ncol)
{
	m_grid_motor_pos = (TSpread*)GetDlgItem(IDC_CUSTOM_MOTOR_POS);
...
int CDialog_Motor_Axis_5::MotorHomingExcution()
{
	...
	switch(m_n_home_step)
	{
		...
		case 110 :
			...
			//	st_msg.mstr_normal_msg = _T("[" + m_str_cmd_motor + "] Motor shut down during transfer.");
				sprintf(st_msg.c_normal_msg, "[%s] Motor shut down during transfer.", m_str_cmd_motor);
...
int CDialog_Motor_Axis_5::MotorMovingExcution()
{
	...
	switch(m_n_move_step)
	{
		...
		case 20 :
			if (st_handler.cwnd_list != NULL)
			{
				...
				sprintf(st_msg.c_normal_msg, "[%s] Motor completed transfer.", m_str_cmd_motor);
				st_handler.cwnd_list->PostMessage(WM_LIST_DATA, 0, NORMAL_MSG);  // 동작 완료 출력 요청
			}
...
void CDialog_Motor_Axis_5::OnButtonControl(BOOL b_flag)
{
	
}


void CDialog_Motor_Axis_5::OnPaint() 
{
...
void CDialog_Motor_Axis_5::Init_Grid_Info()
{
	int   i, j;
	CString str_tmp;
...
/*
BOOL CDialog_Motor_Axis_5::DestroyWindow() 
{
...
void CDialog_Motor_Axis_5::OnDestroy() 
{
	Data_HistoryLog();
	Data_Apply();
...
void CDialog_Motor_Axis_5::OnBtnEmg() 
{
...
// **********************************************************************
LONG CDialog_Motor_Axis_5::OnUserLButtonDown(WPARAM wParam, LPARAM lParam)
{
	CDialog_Message msg_dlg;
	CString strTmp;
	int		nRet;
 ...
 // 		::PostMessage(st_handler.hWnd, WM_MAINFRAME_WORK, 1000, 0);
	...
	{ // "Jog (-)" 버튼이 눌린 경우...
		if(m_n_move_mode == 0)
		{
			...
			{ 
				//st_msg.mstr_abnormal_msg.Format("[조그 - 이동 에러]");
				sprintf(st_msg.c_abnormal_msg, "[조그 - 이동 에러]");
				if(st_handler.mn_language == LANGUAGE_ENGLISH) 
				{
					sprintf(st_msg.c_abnormal_msg, "Jog-go error");
				}
...
LONG CDialog_Motor_Axis_5::OnUserLButtonUp(WPARAM wParam, LPARAM lParam)
{	
	CDialog_Message msg_dlg;
	CString strTmp;
	int		nRet;
...
void CDialog_Motor_Axis_5::OnBtnHome() 
{
	CString strTmp;
...
void CDialog_Motor_Axis_5::OnBtnApply() 
{
	Data_HistoryLog();
	Data_Apply();
}
