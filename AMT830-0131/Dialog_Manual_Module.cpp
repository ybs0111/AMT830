...
////////////////////////////////////////////////////
	...
	SetTimer(TMR_IO_MONITOR_CMD, 50, NULL);		// IO 정보 읽기 타이머 생성
...
void CDialog_Manual_Module::OnBtnGoTrayPos() 
{
	...
	md_mot_start_pos = Func.XYRobot_MovePosValCheck(LDMODULE_SITE, 0, mn_FirstPicker_Y_Pos, n_tray_y); // 트레이에서 찾은 위치로 레일을 이동시킨다
	
	SetTimer(TMR_MODULEMOT_CMD, 10, NULL);		// 모터 동작 제어 타이머
	// **************************************************************************	
}
...
//	int nRet[5];
	...
	if (nIDEvent == TMR_MODULEMOT_CMD) 
	{
		...
		{
			KillTimer(TMR_MODULEMOT_CMD) ;  // 타이머 삭제
			mn_cmd_no = 0;				// 실행 명령 번호 저장 변수 초기화
...
void CDialog_Manual_Module::OnBtnHome() 
{
	...
	SetTimer(TMR_MODULEMOT_CMD, 100, NULL);			// 모터 동작 제어 타이머	
}
...
void CDialog_Manual_Module::OnBtnGoStepRbtMove() 
{
	...
	SetTimer(TMR_STEPMOT_CMD, 100, NULL);			// 모터 동작 제어 타이머	
