...
void CxShadeButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
    ...
    hdcMem.SelectObject(hBitmap); //select the destination for MemDC

	GetWindowText(sCaption);							// get button text
	pDC->SetBkMode(TRANSPARENT);
	// with MemDC we need to select the font...
//	pDC->SelectObject(GetFont());	 //get text font
...
void CxShadeButton::SetShade(UINT shadeID,BYTE granularity,BYTE highlight,BYTE coloring,COLORREF color)
{
	...
	for(i=0;i<129;i++){
		r=((128-i)*GetRValue(locr)+i*GetRValue(midcr))/180;
		g=((128-i)*GetGValue(locr)+i*GetGValue(midcr))/180;
		b=((128-i)*GetBValue(locr)+i*GetBValue(midcr))/180;
	...
	for(i=1;i<129;i++){
		r=((128-i)*GetRValue(midcr)+i*GetRValue(hicr))/180;
		g=((128-i)*GetGValue(midcr)+i*GetGValue(hicr))/180;
		b=((128-i)*GetBValue(midcr)+i*GetBValue(hicr))/180;
...
/////////////////////////////////////////////////////////////////////////////
void CxShadeButton::SetIcon(UINT nID, UINT nIcon, UINT nIconAlign, UINT nIconDown)
{
...
//	hdr.code = BUTTON_MOUSE_DN;
//	GetParent()->SendMessage(WM_NOTIFY, GetDlgCtrlID(), (LPARAM)&hdr);
//	GetParent()->SendMessage(WM_NOTIFY, BUTTON_MOUSE_DN, (LPARAM)&hdr);

    //Default-process the message
	GetParent()->PostMessage(WMU_CHILD_LBUTTON_DOWN, m_ID, (LPARAM)this);
...
//	hdr.hwndFrom = m_hWnd;
//	hdr.idFrom = (UINT)GetDlgCtrlID();
//	hdr.code = BUTTON_MOUSE_UP;
//	GetParent()->SendMessage(WM_NOTIFY, GetDlgCtrlID(), (LPARAM)&hdr);
//	GetParent()->SendMessage(WM_NOTIFY, GetDlgCtrlID(), (LPARAM)&hdr);
	GetParent()->PostMessage(WMU_CHILD_LBUTTON_UP, m_ID, (LPARAM)this);
    //Default-process the message
    m_button_down = false;
